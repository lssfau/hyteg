...
struct ContainsPointFunctor
{
   ...
   bool operator()(const size_t particleIdx, const convection_particles::data::Sphere& sphere, const ParticleAccessor_T& ac, const Vector3<real_t>& point )
   {
   ...
   template<typename ParticleAccessor_T>
   bool operator()(const size_t particleIdx, const convection_particles::data::HalfSpace& halfSpace, const ParticleAccessor_T& ac, const Vector3<real_t>& point )
   {
      static_assert(std::is_base_of<convection_particles::data::IAccessor, ParticleAccessor_T>::value, "Provide a valid accessor as template");
