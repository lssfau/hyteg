...
class P2ToP1ElementwiseDoFValueOperator : public Operator< P2Function< real_t >, P1Function< real_t > >, public P2ToP1FormHyTeG
{
 public:
   P2ToP1ElementwiseDoFValueOperator( const std::shared_ptr< PrimitiveStorage >& storage,
                                      size_t                                     minLevel,
                                      size_t                                     maxLevel,
                                      Functions&... funcs )
            ...
            for ( const auto& cType : celldof::allCellTypes )
            {
               for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
               {
                  localMatrixAssembly3D(
                      mat, cell, level, micro, cType, srcVertexIndices, srcEdgeIndices, dstVertexIndices, alpha );
               }
            }
         }
      }
            ...
            for ( const auto& fType : facedof::allFaceTypes )
            {
               for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
               {
                  localMatrixAssembly2D(
                      mat, face, level, micro, fType, srcVertexIndices, srcEdgeIndices, dstVertexIndices, alpha );
               }
            }
         }
      }
 ...
 protected:
   ...
   /// localElementMatrices2D_[macroCellID][level][cellIdx] = Matrixr< 3, 6 >
   std::map< PrimitiveID, std::map< uint_t, std::vector< Matrixr< 3, 6 >, Eigen::aligned_allocator< Matrixr< 3, 6 > > > > >
       localElementMatrices2D_;

   /// Pre-computed local element matrices.
   /// localElementMatrices3D_[macroCellID][level][cellIdx] = Matrixr< 4, 10 >
