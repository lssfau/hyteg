...
class P1ElementwiseDoFValueOperator : public Operator< P1Function< real_t >, P1Function< real_t > >,
                                      public P1FormHyTeG,
                                      public WeightedJacobiSmoothable< P1Function< real_t > >,
                                      public OperatorWithInverseDiagonal< P1Function< real_t > >
{
 public:
   P1ElementwiseDoFValueOperator( const std::shared_ptr< PrimitiveStorage >& storage,
                                  size_t                                     minLevel,
                                  size_t                                     maxLevel,
   ...
   P1ElementwiseDoFValueOperator( const std::shared_ptr< PrimitiveStorage >& storage,
                                  ...
                                  size_t                                     maxLevel,
                                  Functions&... funcs )
   : P1ElementwiseDoFValueOperator( storage, minLevel, maxLevel, false, funcs... )
   {}
                      ...
                      DoFType                     flag ) const override
   {
      ...
      if ( storage_->hasGlobalCells() )
      {
         ...
         for ( auto& macroIter : storage_->getCells() )
         {
            ...
            Matrixr< 4, 4 > elMat = Matrixr< 4, 4 >::Zero();
      ...
      else
      {
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            Matrixr< 3, 3 > elMat = Matrixr< 3, 3 >::Zero();
   ...
   virtual void smooth_jac( const P1Function< real_t >& dst,
                            const P1Function< real_t >& rhs,
                            const P1Function< real_t >& src,
                            real_t                      omega,
                            size_t                      level,
      ...
      if ( storage_->hasGlobalCells() )
      {
         ...
         for ( auto& macroIter : storage_->getCells() )
         {
            ...
            for ( const auto& cType : celldof::allCellTypes )
            {
               for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
               {
                  localMatrixAssembly3D( mat, cell, level, micro, cType, srcVertexIndices, dstVertexIndices, alpha );
               }
            }
         }
      }
      else
      {
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            for ( const auto& fType : facedof::allFaceTypes )
            {
               for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
               {
                  localMatrixAssembly2D( mat, face, level, micro, fType, srcVertexIndices, dstVertexIndices, alpha );
               }
            }
         }
      }
         ...
         if ( storage_->hasGlobalCells() )
         {
            ...
            for ( const auto& it : storage_->getCells() )
            {
               ...
               for ( const auto& cType : celldof::allCellTypes )
               {
                  for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
                  {
                     Matrixr< 4, 4 >& elMat = localElementMatrix3D( *cell, level, micro, cType );
                     elMat.setZero();
                     assembleLocalElementMatrix3D( *cell, level, micro, cType, elMat );
                  }
               }
         ...
         else
         {
            ...
            for ( const auto& it : storage_->getFaces() )
            {
               ...
               for ( const auto& fType : facedof::allFaceTypes )
               {
                  for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
                  {
                     Matrixr< 3, 3 >& elMat = localElementMatrix2D( *face, level, micro, fType );
                     elMat.setZero();
                     assembleLocalElementMatrix2D( *face, level, micro, fType, elMat );
                  }
               }
   ...
   virtual std::shared_ptr< P1Function< real_t > > getDiagonalValues() const
   {
   ...
   virtual std::shared_ptr< P1Function< real_t > > getInverseDiagonalValues() const override
   {
      WALBERLA_CHECK_NOT_NULLPTR(
   ...
   virtual std::shared_ptr< P1Function< real_t > > getLumpedDiagonalValues() const
   {
      WALBERLA_CHECK_NOT_NULLPTR(
   ...
   virtual std::shared_ptr< P1Function< real_t > > getLumpedInverseDiagonalValues() const
   {
      WALBERLA_CHECK_NOT_NULLPTR(
 ...
 protected:
      ...
      for ( uint_t level = minLevel_; level <= maxLevel_; level++ )
      {
         ...
         else
         {
            ...
            for ( auto& it : storage_->getFaces() )
            {
               ...
               // get hold of the actual data
               PrimitiveDataID< FunctionMemory< real_t >, Face > diagVertexDoFIdx = targetFunction->getFaceDataID();
               ...
               for ( const auto& fType : facedof::allFaceTypes )
               {
                  for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
                  {
                     computeLocalDiagonalContributions2D( face, level, micro, fType, diagVertexData, alpha, lumped );
                  }
               }
            }
   ...
   /// localElementMatrices2D_[macroCellID][level][cellIdx] = Matrixr< 3, 3 >
   std::map< PrimitiveID, std::map< uint_t, std::vector< Matrixr< 3, 3 >, Eigen::aligned_allocator< Matrixr< 3, 3 > > > > >
       localElementMatrices2D_;

   /// Pre-computed local element matrices.
   /// localElementMatrices3D_[macroCellID][level][cellIdx] = Matrixr< 4, 4 >
   std::map< PrimitiveID, std::map< uint_t, std::vector< Matrixr< 4, 4 >, Eigen::aligned_allocator< Matrixr< 4, 4 > > > > >
