...
///
/// The gravity vector is defined via FEM functions. For the annulus / spherical shell:
/// g := -x / norm(x).
///
...
///
/// In a TALA setting you can use the following definitions in HyTeG:
///
...
///
///
/// std::function< real_t( const Point3D&, real_t ) > adiabatic_heat_supg_scaling = [&]( const Point3D& x, real_t u_abs ) {
///     return delta( x, u_abs ) * Di * alpha / C_p;
/// };
///
/// Weak formulation:
///
...
///     scaling: arbitrary std::function coefficient
///
///     - ∫ scaling T (u · g) ( u · ∇w )
///
/// Blending: True
/// Quadrature degree: 5
class p2_adiabatic_heat_u_p2_supg_blending_q5_ElementwiseOperator
: public P2ElementwiseDoFValueOperator< P2Function< real_t >, P2Function< real_t >, P2Function< real_t > >
{
 ...
 public:
   p2_adiabatic_heat_u_p2_supg_blending_q5_ElementwiseOperator(
       ...
       std::function< real_t( const Point3D&, real_t ) > adiabatic_heat_supg_scaling )
   : p2_adiabatic_heat_u_p2_supg_blending_q5_ElementwiseOperator( storage,
                                                                  minLevel,
                                                                  maxLevel,
                                                                  false,
                                                                  uX,
   ...
   , gY_( gY )
   , gZ_( gZ )
   , adiabatic_heat_supg_scaling_( adiabatic_heat_supg_scaling )
   {
 ...
 private:
   ...
   std::function< real_t( const Point3D&, real_t ) > adiabatic_heat_supg_scaling_;
};
