...
class P2ElementwiseDoFValueOperator : public Operator< P2Function< real_t >, P2Function< real_t > >,
                                      public P2FormHyTeG,
                                      public WeightedJacobiSmoothable< P2Function< real_t > >,
                                      public OperatorWithInverseDiagonal< P2Function< real_t > >
{
 public:
   P2ElementwiseDoFValueOperator( const std::shared_ptr< PrimitiveStorage >& storage,
                                  size_t                                     minLevel,
                                  size_t                                     maxLevel,
   ...
   , functuple_( funcs... )
   {
      ...
      {
         // this only works if we overrite the integrate functions directly inside of this class
         // otherwise call it from the inheriting subclass (e.g. in the constructor of the subclass)
   ...
   P2ElementwiseDoFValueOperator( const std::shared_ptr< PrimitiveStorage >& storage,
                                  ...
                                  size_t                                     maxLevel,
                                  Functions&... funcs )
   : P2ElementwiseDoFValueOperator( storage, minLevel, maxLevel, false, funcs... )
   {}
                      ...
                      DoFType                     flag ) const override
   {
      ...
      if ( storage_->hasGlobalCells() )
      {
         ...
         for ( auto& macroIter : storage_->getCells() )
         {
            ...
            Matrixr< 10, 10 > elMat = Matrixr< 10, 10 >::Zero();
            ...
            for ( const auto& cType : celldof::allCellTypes )
            {
               for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
               {
                  ...
                  localMatrixVectorMultiply3D(
                      level, micro, cType, srcVertexData, srcEdgeData, dstVertexData, dstEdgeData, elMat, alpha );
               }
            }
         }
      ...
      else
      {
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            Matrixr< 6, 6 > elMat = Matrixr< 6, 6 >::Zero();
            ...
            for ( const auto& fType : facedof::allFaceTypes )
            {
               for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
               {
                  ...
                  localMatrixVectorMultiply2D(
                      level, micro, fType, srcVertexData, srcEdgeData, dstVertexData, dstEdgeData, elMat, alpha );
               }
            }
         }
   ...
   virtual void apply( const P2Function< real_t >& src,
                       const P2Function< real_t >& dst,
                       size_t                      level,
                       DoFType                     flag,
   ...
   virtual void smooth_jac_scaled( const real_t&               alpha,
                                   const P2Function< real_t >& dst,
   ...
   virtual void smooth_jac( const P2Function< real_t >& dst,
                            const P2Function< real_t >& rhs,
                            const P2Function< real_t >& src,
                            real_t                      omega,
                            size_t                      level,
            ...
            for ( const auto& cType : celldof::allCellTypes )
            {
               for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
               {
                  localMatrixAssembly3D(
                      mat, cell, level, micro, cType, srcVertexIndices, srcEdgeIndices, dstVertexIndices, dstEdgeIndices, alpha );
               }
            }
         }
      }
            ...
            for ( const auto& fType : facedof::allFaceTypes )
            {
               for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
               {
                  localMatrixAssembly2D(
                      mat, face, level, micro, fType, srcVertexIndices, srcEdgeIndices, dstVertexIndices, dstEdgeIndices, alpha );
               }
            }
         }
      }
         ...
         if ( storage_->hasGlobalCells() )
         {
            ...
            for ( const auto& it : storage_->getCells() )
            {
               ...
               for ( const auto& cType : celldof::allCellTypes )
               {
                  for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
                  {
                     Matrixr< 10, 10 >& elMat = localElementMatrix3D( *cell, level, micro, cType );
                     elMat.setZero();
                     assembleLocalElementMatrix3D( *cell, level, micro, cType, elMat );
                  }
               }
         ...
         else
         {
            ...
            for ( const auto& it : storage_->getFaces() )
            {
               ...
               for ( const auto& fType : facedof::allFaceTypes )
               {
                  for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
                  {
                     Matrixr< 6, 6 >& elMat = localElementMatrix2D( *face, level, micro, fType );
                     elMat.setZero();
                     assembleLocalElementMatrix2D( *face, level, micro, fType, elMat );
                  }
               }
   ...
   virtual void computeDiagonalOperatorValues() { computeDiagonalOperatorValues( false, false, real_c( 1 ) ); }

   /// Trigger (re)computation of inverse diagonal matrix entries (central operator weights)
   /// Allocates the required memory if the function was not yet allocated.
   virtual void computeInverseDiagonalOperatorValues() override { computeDiagonalOperatorValues( true, false, real_c( 1 ) ); }

   virtual void computeLumpedDiagonalOperatorValues() { computeDiagonalOperatorValues( false, true, real_c( 1 ) ); }
   virtual void computeLumpedInverseDiagonalOperatorValues() { computeDiagonalOperatorValues( true, true, real_c( 1 ) ); }

   /// Trigger (re)computation of scaled inverse diagonal matrix entries (central operator weights)
   /// Allocates the required memory if the function was not yet allocated.
   virtual void computeInverseDiagonalOperatorValuesScaled( const real_t& alpha ) override
   {
      computeDiagonalOperatorValues( true, false, alpha );
   }

   virtual void computeLumpedInverseDiagonalOperatorValuesScaled( const real_t& alpha )
   {
   ...
   /// Allocates the required memory if the function was not yet allocated.
   virtual void computeDiagonalOperatorValuesScaled( const real_t& alpha )
   {
      computeDiagonalOperatorValues( false, false, alpha );
   }

   virtual void computeLumpedDiagonalOperatorValuesScaled( const real_t& alpha )
   {
      computeDiagonalOperatorValues( false, true, alpha );
   }

   virtual std::shared_ptr< P2Function< real_t > > getDiagonalValues() const
   {
   ...
   virtual std::shared_ptr< P2Function< real_t > > getInverseDiagonalValues() const override
   {
      WALBERLA_CHECK_NOT_NULLPTR(
   ...
   virtual std::shared_ptr< P2Function< real_t > > getLumpedDiagonalValues() const
   {
      WALBERLA_CHECK_NOT_NULLPTR(
   ...
   virtual std::shared_ptr< P2Function< real_t > > getLumpedInverseDiagonalValues() const
   {
      WALBERLA_CHECK_NOT_NULLPTR(
 ...
 protected:
                                             ...
                                             bool                    lumped )
   {
      ...
      if ( !lumped )
      {
         ...
         for ( int k = 3; k < 6; k++ )
         {
            EdgeData[edgeDoFIndices[uint_c( k - 3 )]] += elMat( k, k ) * alpha;
         }
      }
      else
      {
         ...
         for ( int k = 3; k < 6; k++ )
         {
            EdgeData[edgeDoFIndices[uint_c( k - 3 )]] += elMat.colwise().sum()[k] * alpha;
         }
      }
   }
                                             ...
                                             bool                    lumped )
   {
      ...
      if ( !lumped )
      {
         ...
         for ( int k = 4; k < 10; k++ )
         {
            EdgeData[edgeDoFIndices[uint_c( k - 4 )]] += elMat( k, k ) * alpha;
         }
      }
      else
      {
         ...
         for ( int k = 4; k < 10; k++ )
         {
            EdgeData[edgeDoFIndices[uint_c( k - 4 )]] += elMat.colwise().sum()[k] * alpha;
         }
      }
   }
                               ...
                               const real_t&                               alpha ) const
   {
      ...
      for ( uint_t k = 3; k < 6; k++ )
      {
         ...
         colIdx[k] = uint_c( srcEdgeIdx[edgeDoFIndices[k - 3]] );
      }
   ...
   void computeDiagonalOperatorValues( bool invert, bool lumped, const real_t& alpha )
   {
      ...
      if ( invert )
      {
         ...
         else
         {
            ...
            {
               lumpedInverseDiagonalValues_ =
                   std::make_shared< P2Function< real_t > >( "lumped inverse diagonal entries", storage_, minLevel_, maxLevel_ );
            }
      ...
      else
      {
         ...
         else
         {
            ...
            {
               lumpedDiagonalValues_ =
                   std::make_shared< P2Function< real_t > >( "lumped diagonal entries", storage_, minLevel_, maxLevel_ );
            }
      ...
      for ( uint_t level = minLevel_; level <= maxLevel_; level++ )
      {
         ...
         if ( storage_->hasGlobalCells() )
         {
            ...
            for ( auto& macroIter : storage_->getCells() )
            {
               ...
               for ( const auto& cType : celldof::allCellTypes )
               {
                  for ( const auto& micro : celldof::macrocell::Iterator( level, cType, 0 ) )
                  {
                     computeLocalDiagonalContributions3D(
                         cell, level, micro, cType, diagVertexData, diagEdgeData, alpha, lumped );
                  }
               }
            }
         ...
         else
         {
            ...
            for ( auto& it : storage_->getFaces() )
            {
               ...
               // get hold of the actual data
               PrimitiveDataID< FunctionMemory< real_t >, Face > diagVertexDoFIdx =
                   targetFunction->getVertexDoFFunction().getFaceDataID();
               PrimitiveDataID< FunctionMemory< real_t >, Face > diagEdgeDoFIdx =
                   targetFunction->getEdgeDoFFunction().getFaceDataID();
               real_t* diagVertexData = face.getData( diagVertexDoFIdx )->getPointer( level );
               real_t* diagEdgeData   = face.getData( diagEdgeDoFIdx )->getPointer( level );
               ...
               for ( const auto& fType : facedof::allFaceTypes )
               {
                  for ( const auto& micro : facedof::macroface::Iterator( level, fType, 0 ) )
                  {
                     computeLocalDiagonalContributions2D(
                         face, level, micro, fType, diagVertexData, diagEdgeData, alpha, lumped );
                  }
               }
            }
   ...
   /// localElementMatrices2D_[macroCellID][level][cellIdx] = Matrixr< 6, 6 >
   std::map< PrimitiveID, std::map< uint_t, std::vector< Matrixr< 6, 6 >, Eigen::aligned_allocator< Matrixr< 6, 6 > > > > >
       localElementMatrices2D_;

   /// Pre-computed local element matrices.
   /// localElementMatrices3D_[macroCellID][level][cellIdx] = Matrixr< 10, 10 >
   std::map< PrimitiveID, std::map< uint_t, std::vector< Matrixr< 10, 10 >, Eigen::aligned_allocator< Matrixr< 10, 10 > > > > >
