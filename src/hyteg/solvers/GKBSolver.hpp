...
class GKBSolver : public Solver< SaddlePointOp >
{
 public:
   ...
   GKBSolver( const std::shared_ptr< PrimitiveStorage >& storage,
              uint_t                                     level,
   ...
   void solve( const SaddlePointOp& K, const nmFunction& x, const nmFunction& b, const uint_t level ) override
   {
      ...
      else
      {
         ...
         A.apply( f1, tmp_v, level_, flag );
         f0.assign( { 1, nu }, { f0, tmp_v }, level_, flag );
      }

      timingTree->start( "GKBSolver" );

      // Init: first q
      ALSolver.solve( M, u, f0, level_ );                     // u = M^-1*f
      AT.apply( u, tmp_q, level_, flag );                     // store A'*u
      dualr.assign( { 1, -1 }, { f1, tmp_q }, level_, flag ); // r = g - A'*u
      q.assign( { nu }, { dualr }, level_, flag );            // q = N^-1*r
      beta = sqrt( dualr.dotGlobal( q, level_, flag ) );      // beta = ||r'*q||_2
      q.assign( { 1 / beta }, { q }, level_, flag );          // q = q/beta

      // Init: first v
      A.apply( q, tmp_v, level_, flag );                  // store A*q
      ALSolver.solve( M, v, tmp_v, level_ );              // v = M^-1*A*q
      M.apply( v, tmp_v, level_, flag );                  // for Mnorm of v
      alpha = sqrt( v.dotGlobal( tmp_v, level_, flag ) ); // alpha = ||v||_M
      v.assign( { 1 / alpha }, { v }, level_, flag );     // v = v/alpha

      d.assign( { 1 / alpha }, { q }, level_, flag ); // d = q/alpha
      z.at( 0 ) = ( beta / alpha );                  // z0 = beta/alpha

      // initial iterates of u,p
      u.assign( { 1, z.at( 0 ) }, { u, v }, level_, flag ); // u = u + z0*v
      p.assign( { -z.at( 0 ) }, { d }, level_, flag );      // p = p -z0*d
      ...
      if ( printInfo )
      {
         if ( k == maxIter - 1 )
         {
            WALBERLA_LOG_INFO_ON_ROOT( "GKB did not converge to MErrtol=" << merrorTolerance << " or Rtol=" << resTolerance
      ...
      x.uvw().assign( { 1 }, { u }, level_, flag );
      x.p().assign( { 1 }, { p }, level_, flag );
   }
   ...
   void ComputeNextQ()
   {
      AT.apply( v, tmp_q, level_, flag, Replace );            // Store A'*v
      q.assign( { nu, -alpha }, { tmp_q, q }, level_, flag ); // Store q = 1/gamma*(A'*v -alpha*gamma*q)
      beta = sqrt( q.dotGlobal( q, level_, flag ) * 1 / nu ); // beta = ||q||_N
      q.assign( { 1 / beta }, { q }, level_, flag );          // q = q/beta
   }

   void ComputeNextV()
   {
      A.apply( q, tmp_v, level_, flag, Replace );                   // Store A*q
      M.apply( v, tmp_w, level_, flag, Replace );                   // Store M*V
      tmp_v.assign( { 1, -beta }, { tmp_v, tmp_w }, level_, flag ); // Store (A*q - beta*M*v)
      ALSolver.solve( M, v, tmp_v, level_ );                        // v = M^-1*(A*q - beta*M*v) TODO: Optimize M^-1*M
      M.apply( v, tmp_v, level_, flag, Replace );
      alpha = sqrt( tmp_v.dotGlobal( v, level_, flag ) ); // alpha = ||v||_M
      v.assign( { 1 / alpha }, { v }, level_, flag );     // v = v/alpha
   }
   ...
   void SolutionUpdate()
   {
      ...
      d.assign( { 1 / alpha, -beta / alpha }, { q, d }, level_, flag ); // d = 1/alpha*(q - beta*d)
      u.assign( { 1, z.at( k ) }, { u, v }, level_, flag );             // u = u + z*v
      p.assign( { 1, -z.at( k ) }, { p, d }, level_, flag );            // p = p -z*d
   }
   ...
   {
      if ( k > delay && LowerBoundMErrEstimate() < merrorTolerance )
         return true;

      if ( useResidualConvergence && ResidualNorm( K, b, f0, f1 ) < resTolerance )
         return true;
   ...
   {
      globalX.uvw().assign( { 1 }, { u }, level_, flag );
      globalX.p().assign( { 1 }, { p }, level_, flag );
      M.apply( u, tmp_v, level_, flag );
      A.apply( p, tmp_w, level_, flag );
      globalR.uvw().assign( { 1, -1, -1 }, { f0, tmp_v, tmp_w }, level_, flag );
      AT.apply( u, tmp_q, level_, flag );
      globalR.p().assign( { 1, -1 }, { f1, tmp_q }, level_, flag );

      real_t resnorm = sqrt( globalR.dotGlobal( globalR, level_, flag ) );
      if ( printInfo )
...
// specification of GKB solver for P2P1TaylorHood Finite Elements with CG as inner solver
// now with AL
using GKBSolver_P2P1TH = GKBSolver< P2P1TaylorHoodStokesOperator,
                                    P1ToP2ConstantDivTOperator,
                                    P2ToP1ConstantDivOperator,
                                    ALOP_P2P1TH,
