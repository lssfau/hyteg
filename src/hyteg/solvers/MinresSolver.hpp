...
namespace hyteg {
    ...
    public:
        ...
        MinResSolver(
                ...
                : maxIter_(maxIter), relativeTolerance_(tolerance), absoluteTolerance_(1e-16), printInfo_(false),
                  flag_(hyteg::Inner | hyteg::NeumannBoundary | hyteg::FreeslipBoundary),
                  preconditioner_(preconditioner), p_vm("minres_vm", storage, minLevel, maxLevel),
                  p_v("minres_v", storage, minLevel, maxLevel), p_vp("minres_vp", storage, minLevel, maxLevel),
                  p_z("minres_z", storage, minLevel, maxLevel), p_zp("minres_zp", storage, minLevel, maxLevel),
                  p_wm("minres_wm", storage, minLevel, maxLevel), p_w("minres_w", storage, minLevel, maxLevel),
                  p_wp("minres_wp", storage, minLevel, maxLevel), p_tmp("minres_tmp", storage, minLevel, maxLevel),
            ...
            for (size_t i = 0; i < maxIter_; ++i) {
                ...
                if (printInfo_) {
                    WALBERLA_LOG_INFO_ON_ROOT(
                            walberla::format("[MinRes] iter: %6d | residuum: %10.5e", i, std::abs(eta)));
                }

                if (std::abs(eta) / res_start < relativeTolerance_ || std::abs(eta) < absoluteTolerance_) {
                    if (printInfo_) {
                        WALBERLA_LOG_INFO_ON_ROOT(
                                "[MinRes] converged after " << std::defaultfloat << i << " iterations");
                    }
                    break;
                }
            }
