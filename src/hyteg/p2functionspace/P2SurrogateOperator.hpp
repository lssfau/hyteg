 ...
 public:
   ...
   void interpolateStencils( uint_t polyDegree )
   {
      ...
      for ( auto& itF : storage_->getFaces() )
      {
         ...
         for ( uint_t level = minLevel_; level <= maxLevel_; ++level )
         {
            ...
            P2::StencilInterpolator< LSQPType::EDGE_ALL, P2::NumStencilentries2D::EtE > EtEInterpolator( polyDegree );
            ...
            for ( const auto& it : hyteg::edgedof::macroface::Iterator( interpolationLevel_, 0 ) )
            {
               ...
               P2::variablestencil::macroface::assembleStencil( form_,
                                                                x,
                                                                dirS,
                                                                dirSE,
                                                                dirE,
               ...
               UpdateType                  updateType = Replace ) const override
   {
      ...
      for ( auto& it : storage_->getFaces() )
      {
         ...
         {
            P2::variablestencil::macroface::applyPolynomial( polyDegree_,
                   ...
                   DoFType                     flag ) const override
   {
      ...
      for ( auto& it : storage_->getFaces() )
      {
         ...
         if ( testFlag( faceFlag, flag ) )
         {
            P2::variablestencil::macroface::smoothGSPolynomial( polyDegree_,
                                                                polynomialIDs_.at( level ),
                                                                level,
                                                                face,
                                                                dstVertexDoF.getFaceDataID(),
