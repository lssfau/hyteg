...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackEdgeFromVertex( Edge*                      receiver,
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::packEdgeForVertex( const Edge*                sender,
                                                        const PrimitiveID&         receiver,
                                                        walberla::mpi::SendBuffer& buffer ) const
{
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackVertexFromEdge( Vertex*                    receiver,
                                                           const PrimitiveID&         sender,
                                                           walberla::mpi::RecvBuffer& buffer ) const
{
...
void VertexDoFPackInfo< ValueType >::communicateLocalEdgeToVertex( const Edge* sender, Vertex* receiver ) const
{
   ...
   {
      const uint_t idx = vertexdof::macroedge::indexFromVertex(
          level_, levelinfo::num_microvertices_per_edge( level_ ) - 2, stencilDirection::VERTEX_C );
      vertexData[edgeIdOnVertex + 1] = edgeData[idx];
   }
   else
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::packEdgeForFace( const Edge* sender,
                                                      const PrimitiveID& /*receiver*/,
                                                      walberla::mpi::SendBuffer& buffer ) const
{
   ValueType* edgeData  = sender->getData( dataIDEdge_ )->getPointer( level_ );
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackFaceFromEdge( Face*                      receiver,
                                                         const PrimitiveID&         sender,
                                                         walberla::mpi::RecvBuffer& buffer ) const
{
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::packFaceForEdge( const Face*                sender,
                                                      const PrimitiveID&         receiver,
                                                      walberla::mpi::SendBuffer& buffer ) const
{
   ...
   {
      for ( const auto& it :
            indexing::FaceBoundaryIterator( levelinfo::num_microvertices_per_edge( level_ ) - 1, faceBorderDirection, 1 ) )
      {
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackEdgeFromFace( Edge*                      receiver,
                                                         const PrimitiveID&         sender,
                                                         walberla::mpi::RecvBuffer& buffer ) const
{
   ValueType*   edgeData     = receiver->getData( dataIDEdge_ )->getPointer( level_ );
   ...
   if ( senderFace->getNumNeighborCells() > 0 )
   {
      ...
      {
         buffer >> edgeData[vertexdof::macroedge::indexOnNeighborCell(
                       level_, i, localTopCellIDOnEdge, receiver->getNumNeighborFaces() )];
      }
      ...
      if ( senderFace->getNumNeighborCells() == 2 )
      {
         ...
         {
            buffer >> edgeData[vertexdof::macroedge::indexOnNeighborCell(
                          level_, i, localBottomCellIDOnEdge, receiver->getNumNeighborFaces() )];
         }
      }
   }
}
...
void VertexDoFPackInfo< ValueType >::communicateLocalFaceToEdge( const Face* sender, Edge* receiver ) const
{
   ...
   for ( const auto& it : vertexdof::macroface::BoundaryIterator( level_, faceBorderDirection, 1 ) )
   {
      edgeData[vertexdof::macroedge::indexOnNeighborFace( level_, idx, faceIdOnEdge )] =
   ...
   if ( sender->getNumNeighborCells() > 0 )
   {
      ...
      for ( const auto& it :
            indexing::FaceBoundaryIterator( levelinfo::num_microvertices_per_edge( level_ ) - 1, faceBorderDirection, 1 ) )
      {
         edgeData[vertexdof::macroedge::indexOnNeighborCell(
             level_, idx, localTopCellIDOnEdge, receiver->getNumNeighborFaces() )] =
             faceData[vertexdof::macroface::index( level_, it.col(), it.row(), 0 )];
         idx++;
      }
      // Bottom ghost-layer is only sent if there is a second neighboring cell
      if ( sender->getNumNeighborCells() == 2 )
      {
         ...
         {
            edgeData[vertexdof::macroedge::indexOnNeighborCell(
                level_, idx, localBottomCellIDOnEdge, receiver->getNumNeighborFaces() )] =
                faceData[vertexdof::macroface::index( level_, it.col(), it.row(), 1 )];
            idx++;
         }
      }
   }
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::packFaceForCell( const Face*                sender,
                                                      const PrimitiveID&         receiver,
                                                      walberla::mpi::SendBuffer& buffer ) const
{
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackCellFromFace( Cell*                      receiver,
                                                         const PrimitiveID&         sender,
                                                         walberla::mpi::RecvBuffer& buffer ) const
{
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::packCellForFace( const Cell*                sender,
                                                      const PrimitiveID&         receiver,
                                                      walberla::mpi::SendBuffer& buffer ) const
{
   ...
   for ( const auto& it :
         vertexdof::macrocell::BoundaryIterator( level_, iterationVertex0, iterationVertex1, iterationVertex2, 1 ) )
   {
      buffer << cellData[vertexdof::macrocell::indexFromVertex( level_, it.x(), it.y(), it.z(), stencilDirection::VERTEX_C )];
   }
}
...
template < typename ValueType >
void VertexDoFPackInfo< ValueType >::unpackFaceFromCell( Face*                      receiver,
                                                         const PrimitiveID&         sender,
                                                         walberla::mpi::RecvBuffer& buffer ) const
{
   ValueType* faceData = receiver->getData( dataIDFace_ )->getPointer( level_ );
...
void VertexDoFPackInfo< ValueType >::communicateLocalCellToCell( const Cell* sender, Cell* receiver ) const
{
   ...
   uint_t glMicroVolumeIdx_x = 0;
   uint_t glMicroVolumeIdx_y = 0;
   while ( cellIteratorSenderCellType0 != cellIteratorSenderCellType0.end() )
   {
      ...
      const auto receiverIdx =
          volumedofspace::indexing::indexGhostLayerDirectly( glMicroVolumeIdx_x,
                                                                       glMicroVolumeIdx_y,
      ...
      glMicroVolumeIdx_x++;
      if ( glMicroVolumeIdx_x + glMicroVolumeIdx_y >= levelinfo::num_microvertices_per_edge( level_ ) - 1 )
      {
         glMicroVolumeIdx_x = 0;
         glMicroVolumeIdx_y++;
      }
