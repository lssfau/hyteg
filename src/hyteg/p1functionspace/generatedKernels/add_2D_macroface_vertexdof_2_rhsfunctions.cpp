...
static void add_2D_macroface_vertexdof_2_rhs_functions_level_any(float * RESTRICT _data_p1FaceDst, float * RESTRICT _data_p1FaceSrc0, float * RESTRICT _data_p1FaceSrc1, float c0, float c1, int level)
{
    for (int ctr_2 = 1; ctr_2 < (1 << (level)); ctr_2 += 1)
    {
        // inner triangle
        for (int ctr_1 = 1; ctr_1 < -ctr_2 + (1 << (level)); ctr_1 += 1)
        {
         ...
         const float xi_5 = _data_p1FaceDst[ctr_1 + ctr_2*((1 << (level)) + 2) - ((ctr_2*(ctr_2 + 1)) / (2))];
         _data_p1FaceDst[ctr_1 + ctr_2*((1 << (level)) + 2) - ((ctr_2*(ctr_2 + 1)) / (2))] = xi_3 + xi_4 + xi_5;
        }
    }
}
...
void add_2D_macroface_vertexdof_2_rhs_functions(float * RESTRICT _data_p1FaceDst, float * RESTRICT _data_p1FaceSrc0, float * RESTRICT _data_p1FaceSrc1, float c0, float c1, int level)
{
    switch( level )
    {
