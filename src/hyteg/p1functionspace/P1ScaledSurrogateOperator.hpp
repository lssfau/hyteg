...
class P1ScaledSurrogateOperator : public P1Operator< P1Form >
{
 ...
 public:
   ...
   const PrimitiveDataID< LevelWiseMemory< StencilPoly_face >, Face > getFacePolyID()
   {
      if ( storage_->hasGlobalCells() )
      {
         WALBERLA_LOG_WARNING( "P1ScaledSurrogateOperator::getFacePolyID() called for 3D mesh!" );
      }
      return facePolyID_;
   }
   ...
   const PrimitiveDataID< LevelWiseMemory< StencilPoly_cell >, Cell > getCellPolyID()
   {
      if ( !storage_->hasGlobalCells() )
      {
         WALBERLA_LOG_WARNING( "P1ScaledSurrogateOperator::getCellPolyID() called for 2D mesh!" );
      }
      return cellPolyID_;
   }
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            for ( uint_t j = 1; j < rowsizeY - 2; ++j )
            {
               ...
               for ( uint_t i = 1; i < rowsizeX - 1; ++i )
               {
                  ...
                  real_t centerVal = stencilMemory[vertexdof::stencilIndexFromVertex( stencilDirection::VERTEX_C )];

                  for ( uint_t c = 0; c < stencilSize; ++c )
                  {
                     interpolator[c].addInterpolationPoint( x, stencilMemory[c] / centerVal );
                  }
               }
            }
            ...
            for ( uint_t k = 1; k < rowsizeZ - 3; ++k )
            {
               ...
               for ( uint_t j = 1; j < rowsizeY - 2; ++j )
               {
                  ...
                  for ( uint_t i = 1; i < rowsizeX - 1; ++i )
                  {
                     ...
                     real_t centerVal = stencilMemory[indexing::IndexIncrement( { 0, 0, 0 } )];

                     for ( auto& [idx, val] : stencilMemory )
                     {
                        interpolator[idx].addInterpolationPoint( x, val / centerVal );
                     }
                  }
               }
            }
   ...
   */
   inline void assemble_stencil_face( real_t* face_stencil, const uint_t i, const uint_t j ) const
   {
      ...
      for ( uint_t c = 0; c < facePolyEvaluator_.size(); ++c )
      {
         if constexpr ( USE_INCREMENTAL_EVAL )
         {
            face_stencil[c] = facePolyEvaluator_[c].incrementEval() * scaling;
         }
         else
         {
            face_stencil[c] = facePolyEvaluator_[c].evalX( x ) * scaling;
         }
      }
   }
   ...
   */
                                      ...
                                      const uint_t                        k ) const
   {
      ...
      for ( auto& [idx, evaluator] : cellPolyEvaluator_ )
      {
         if constexpr ( USE_INCREMENTAL_EVAL )
         {
            cell_stencil[idx] = evaluator.incrementEval() * scaling;
         }
         else
         {
            cell_stencil[idx] = evaluator.evalX( x ) * scaling;
         }
      }
   }
...
typedef P1ScaledSurrogateOperator< forms::p1_diffusion_blending_q1 >  P1ScaledSurrogateLaplaceOperator;
typedef P1ScaledSurrogateOperator< forms::p1_mass_blending_q4 >       P1ScaledSurrogateMassOperator;
typedef P1ScaledSurrogateOperator< forms::p1_div_k_grad_blending_q1 > P1ScaledSurrogateDivkGradOperator;
typedef P1ScaledSurrogateOperator< forms::p1_div_k_grad_affine_q1 >   P1ScaledSurrogateAffineDivkGradOperator;
