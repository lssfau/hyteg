...
/// Base class for all HyTeG functions representing vector fields
/// which are *not* made up of individual scalar component functions
/// \sa CSFVectorFunction
...
class VectorFunction
{
 public:
   ...
   VectorFunction( std::string name, const std::shared_ptr< PrimitiveStorage >& storage )
   ...
   VectorFunction( std::string name, const std::shared_ptr< PrimitiveStorage >& storage, uint_t minLevel, uint_t maxLevel )
   : functionName_( name )
   , storage_( storage )
   , minLevel_( minLevel )
   , maxLevel_( maxLevel )
...
template < typename FunctionType >
std::vector< std::string > VectorFunction< FunctionType >::functionNames_ = {};

template < typename FunctionType >
std::map< uint_t, uint_t > VectorFunction< FunctionType >::levelWiseFunctionCounter_ = {};
