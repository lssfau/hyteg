...
namespace hyteg {
    ...
    public:
        ...
        void
        interpolate(const std::function<ValueType(const Point3D &)> &expr, uint_t level, DoFType dofType = All) const {
        ...
        bool evaluate(const Point3D &coordinates, uint_t level, ValueType &value,
                            ...
                            for (const auto &idxIt: facedof::macroface::Iterator(level, faceType)) {
                                ...
                                for (uint_t i = 0; i < 3; i++) {
                                    const auto elementVertex =
                                            vertexdof::macroface::coordinateFromIndex(level, face,
                                                                                      vertexIndicesArray[i]);
                                ...
                                for (uint_t i = 0; i < numDofs; i++) {
                                    vertexDoFIndices[i] =
                                            vertexdof::macroface::index(level, vertexIndicesArray[i].x(),
                                                                        vertexIndicesArray[i].y());
                                }

                                vertexdof::getVertexDoFDataIndicesFromMicroFace(idxIt, faceType, level,
                                                                                vertexDoFIndices);
                ...
                // DGE
                {
                    ...
                    for (auto faceType: facedof::allFaceTypes) {
                        for (const auto &idxIt: facedof::macroface::Iterator(level, faceType)) {
                            ...
                            basis_discontinuous_.integrateBasisFunction(degree, elementVertices, f[0], f[1],
                                                                        dofValues);
                            for (uint_t i = 0; i < numDofs; i++) {
                        ...
                        for (auto cellType: celldof::allCellTypes) {
                            for (const auto &idxIt: celldof::macrocell::Iterator(level, cellType)) {
                                ...
                                for (uint_t i = 0; i < 4; i++) {
                                    const auto elementVertex = vertexdof::macrocell::coordinateFromIndex(level,
                                                                                                         cell,
                                                                                                         vertexIndicesArray[i]);
                                    elementVertices[i](0) = elementVertex[0];
                                    elementVertices[i](1) = elementVertex[1];
                                ...
                                for (uint_t i = 0; i < numDofs; i++) {
                                    vertexDoFIndices[i] =
                                            celldof::macrocell::index(level, vertexIndicesArray[i].x(),
                                                                      vertexIndicesArray[i].y(),
                                                                      vertexIndicesArray[i].z(), cellType);
                                }

                                vertexdof::getVertexDoFDataIndicesFromMicroCell(idxIt, cellType, level,
                                                                                vertexDoFIndices);
                                basis_conforming_.integrateBasisFunction(degree, elementVertices, f[d], dofValues);
                                for (uint_t i = 0; i < numDofs; i++) {
                                    dofs[vertexDoFIndices[i]] += ValueType(dofValues[i]);
                ...
                // DGE
                {
                    ...
                    for (auto cellType: celldof::allCellTypes) {
                        for (const auto &idxIt: celldof::macrocell::Iterator(level, cellType)) {
                            ...
                            basis_discontinuous_.integrateBasisFunction(degree, elementVertices, f[0], f[1], f[2],
                                                                        dofValues);
                            for (uint_t i = 0; i < numDofs; i++) {
                                dofs[volumedofspace::indexing::index(idxIt.x(), idxIt.y(), idxIt.z(), cellType, i,
                                                                     numDofs,
                                                                     level,
                                                                     memLayout)] +=
                                        ValueType(dofValues[i]);
        ...
        void evaluateOnMicroElement(const Point3D &coordinates,
            ...
            auto dof_discontinuous =
                    u_discontinuous_->getDGFunction()->volumeDoFFunction()->dof(faceID, elementIndex, 0, faceType,
                                                                                level);
            real_t value_discontinuous =
                    ValueType(dof_discontinuous) * (coordinates[componentIdx] - midpoint[componentIdx]);
    ...
    protected:
        ...
        static void
        applyDirichletBC(const std::shared_ptr<dg::DGForm> &form, const P1Function<ValueType> &f, const uint_t level) {
                            ...
                            for (uint_t n = 0; n < 4; n++) {
                                if (neighborInfo.atMacroBoundary(n) &&
                                    ...
                                    for (uint_t dofIdx = 0; dofIdx < numDofs; dofIdx++) {
                                        const uint_t p1Index = vertexdof::macrocell::index(
                                                level, vertexDoFIndicesArray[dofIdx].x(),
                                                vertexDoFIndicesArray[dofIdx].y(), vertexDoFIndicesArray[dofIdx].z());
    ...
    EGFunction<ValueType>::EGFunction(const std::string &name,
            ...
            : Function<EGFunction<ValueType> >(name, storage, minLevel, maxLevel),
              u_conforming_{std::make_shared<P1VectorFunction<ValueType> >(name + "_conforming",
                                                                           ...
                                                                           maxLevel,
                                                                           bc,
                                                                           true)}, u_discontinuous_{
                    std::make_shared<P0Function<ValueType> >(name + "_discontinuous", storage, minLevel, maxLevel, bc)},
              basis_conforming_(), basis_discontinuous_() {}
