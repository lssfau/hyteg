...
class StokesConvergenceOrderTest
{
 public:
   ...
   , writeVTK_( writeVTK )
   , runUntilErrorNorm_( runUntilErrorNorm )
   , checkSolution_( checkSolution )
   , discrErrors_( discrErrors )
   , resNorms_( resNorms )
   {
      ...
      {
         ratesCounter.update( RunStokesTestOnLevel( level ), MeshQuality::getMaximalEdgeLength( storage_, level ), testName );
   ...
   class EGConvRatesCounter
   {
    public:
      ...
      void update( const ErrorArray& newErrors, real_t h_new, const std::string& fname )
      {
         ...
         std::transform( newErrors.begin() + 1, newErrors.end(), errors_.begin(), rates_.begin(), std::divides< real_t >() );
      ...
      {
         WALBERLA_LOG_INFO_ON_ROOT( walberla::format( "%6d|%15.2e|%15.2e|%15.2e|%15.2e|%15.2e|%15.2e|%15.2e|",
                                                      level,
                                                      currentDoFs_,
                                                      errors_[0],
                                                      errors_[1],
      ...
      {
         WALBERLA_LOG_INFO_ON_ROOT( "Convergence L2 rate on level " << level << ": " << rates_[0] << ", expected rate: ["
         ...
         WALBERLA_CHECK_GREATER_EQUAL( rates_[0],
                                       expectedL2VeloRate_ - 0.1,
                                       "Convergence L2 rate on level " << level << " too small (computed: " << rates_[0]
                                                                       << ", expected - eps: " << expectedL2VeloRate_ - 0.1
                                                                       << ")" );
      }
   ...
   void integrateRHS( uint_t level, StokesFunctionType& rhs )
   {
      ...
      if constexpr ( std::is_same< StokesOperatorType, EGP0StokesOperatorNitscheBC >::value )
      {
         ...
         else
         {
            ...
            rhs.uvw().applyDirichletBoundaryConditions( laplaceForm0, laplaceForm1, laplaceForm2, laplaceFormDG, level );
         }
      }
      else
      {
         ...
         else
         {
            ...
            // Assemble RHS by integration, apply bcs weakly
            //   rhs.uvw().evaluateLinearFunctional( rhsFuncX, rhsFuncY, rhsFuncZ, level );
            rhs.uvw().applyDirichletBoundaryConditions( epsForm00, epsForm11, epsForm22, epsFormDG, level );
         }
      }
   ...
   ErrorArray RunStokesTestOnLevel( const uint_t& level )
   {
      ...
      case 2: {
         PETScBlockPreconditionedStokesSolver< StokesOperatorType > solver( storage_,
                                                                            level,
                                                                            resNorms_ != NULL ? resNorms_->at( level - 2 ) : 1e-4,
         ...
         if ( runUntilErrorNorm_ )
         {
            ...
            uint_t attempts    = 0;
            WALBERLA_LOG_INFO_ON_ROOT( "Discretization error is " << discrErrors_->at( level - 4 ) << ", current error is "
                                                                  << L2VeloError << "." );

            while ( attempts < 100 && L2VeloError > discrErrors_->at( level - 4 ) + discrErrors_->at( level - 4 ) / 2 )
            {
               WALBERLA_LOG_INFO_ON_ROOT( "Discretization error (" << discrErrors_->at( level - 4 ) << ") not reached ("
      ...
      if ( writeVTK_ )
      {
         VTKOutput vtk( "/mnt/c/Users/Fabia/OneDrive/Desktop/hyteg_premerge_2/hyteg/output", testName_, storage_ );
         if constexpr ( isEGP0Discr< StokesOperatorType >() )
         {
            vtk.add( u );
