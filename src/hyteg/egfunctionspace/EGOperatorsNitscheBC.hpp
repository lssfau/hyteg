...
namespace hyteg {
    namespace dg {
        namespace eg {
            ...
            public:
                EGLaplaceOperatorNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage, uint_t minLevel,
                                           uint_t maxLevel)
                        : Operator<EGFunction<real_t>, EGFunction<real_t> >(storage, minLevel, maxLevel),
                          ...
                          dg_to_dg_coupling_(storage, minLevel, maxLevel,
                                             std::make_shared<EGVectorLaplaceFormNitscheBC_EE>()) {}
                ...
                void apply(const EGFunction<real_t> &src,
                    ...
                    dg_to_cg_coupling_.apply(*src.getDiscontinuousPart(), *dst.getConformingPart(), level, flag,
                                             updateType);
                    cg_.apply(src.getConformingPart()->component(0), dst.getConformingPart()->component(0), level, flag,
                              Add);
                    cg_.apply(src.getConformingPart()->component(1), dst.getConformingPart()->component(1), level, flag,
                              Add);
                    if (src.getDimension() == 3) {
                        cg_.apply(src.getConformingPart()->component(2), dst.getConformingPart()->component(2), level,
                                  flag, Add);
                    }
                    cg_to_dg_coupling_.apply(*src.getConformingPart(), *dst.getDiscontinuousPart(), level, flag,
                                             updateType);
                    dg_to_dg_coupling_.apply(*src.getDiscontinuousPart(), *dst.getDiscontinuousPart(), level, flag,
                                             Add);
                }
                ...
                void toMatrix(const std::shared_ptr<SparseMatrixProxy> &mat,
                    ...
                    cg_.toMatrix(mat, src.getConformingPart()->component(0), dst.getConformingPart()->component(0),
                                 level, flag);
                    cg_.toMatrix(mat, src.getConformingPart()->component(1), dst.getConformingPart()->component(1),
                                 level, flag);
                    if (src.getDimension() == 3) {
                        cg_.toMatrix(mat, src.getConformingPart()->component(2), dst.getConformingPart()->component(2),
                                     level, flag);
                    }
                    dg_to_cg_coupling_.toMatrix(mat, *src.getDiscontinuousPart(), *dst.getConformingPart(), level,
                                                flag);
                    cg_to_dg_coupling_.toMatrix(mat, *src.getConformingPart(), *dst.getDiscontinuousPart(), level,
                                                flag);
                    dg_to_dg_coupling_.toMatrix(mat, *src.getDiscontinuousPart(), *dst.getDiscontinuousPart(), level,
                                                flag);
                }
...
// TODO: fix code duplication
            ...
            public:
                EGEpsilonOperatorNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage,
                        ...
                        : Operator<EGFunction<real_t>, EGFunction<real_t> >(storage, minLevel, maxLevel),
                          viscosity_(viscosity), cg_00(storage, minLevel, maxLevel,
                                                       std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_00>(viscosity)),
                          cg_01(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_01>(viscosity)),
                          cg_02(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_02>(viscosity)),
                          cg_10(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_10>(viscosity)),
                          cg_11(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_11>(viscosity)),
                          cg_12(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_12>(viscosity)),
                          cg_20(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_20>(viscosity)),
                          cg_21(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_21>(viscosity)),
                          cg_22(storage, minLevel, maxLevel,
                                std::make_shared<eg::EGEpsilonFormNitscheBC_P1P1_22>(viscosity))
                        // , cg_(storage, minLevel, maxLevel, viscosity)
                        , cg_to_dg_coupling_(storage,
                                             minLevel,
                                             maxLevel,
                                             std::make_tuple(
                                                     std::make_shared<typename CGToDGOperatorType::OperX_T::FormType>(
                                                             viscosity),
                                                     std::make_shared<typename CGToDGOperatorType::OperY_T::FormType>(
                                                             viscosity),
                                                     std::make_shared<typename CGToDGOperatorType::OperZ_T::FormType>(
                                                             viscosity))), dg_to_cg_coupling_(storage,
                                                                                              minLevel,
                                                                                              maxLevel,
                                                                                              std::make_tuple(
                                                                                                      std::make_shared<typename DGToCGOperatorType::OperX_T::FormType>(
                                                                                                              viscosity),
                                                                                                      std::make_shared<typename DGToCGOperatorType::OperY_T::FormType>(
                                                                                                              viscosity),
                                                                                                      std::make_shared<typename DGToCGOperatorType::OperZ_T::FormType>(
                                                                                                              viscosity))),
                          dg_to_dg_coupling_(storage, minLevel, maxLevel,
                                             std::make_shared<EGEpsilonFormNitscheBC_EE>(viscosity)) {}
                ...
                void apply(const EGFunction<real_t> &src,
                    ...
                    cg_00.apply(src.getConformingPart()->component(0), dst.getConformingPart()->component(0), level,
                                flag, Add);
                    cg_01.apply(src.getConformingPart()->component(1), dst.getConformingPart()->component(0), level,
                                flag, Add);
                    cg_10.apply(src.getConformingPart()->component(0), dst.getConformingPart()->component(1), level,
                                flag, Add);
                    cg_11.apply(src.getConformingPart()->component(1), dst.getConformingPart()->component(1), level,
                                flag, Add);
                ...
                void toMatrix(const std::shared_ptr<SparseMatrixProxy> &mat,
                    ...
                    cg_00.toMatrix(mat, src.getConformingPart()->component(0), dst.getConformingPart()->component(0),
                                   level, flag);
                    cg_01.toMatrix(mat, src.getConformingPart()->component(1), dst.getConformingPart()->component(0),
                                   level, flag);
                    cg_10.toMatrix(mat, src.getConformingPart()->component(0), dst.getConformingPart()->component(1),
                                   level, flag);
                    cg_11.toMatrix(mat, src.getConformingPart()->component(1), dst.getConformingPart()->component(1),
                                   level, flag);

                    if (src.getDimension() == 3) {
                        cg_02.toMatrix(mat, src.getConformingPart()->component(2),
                                       dst.getConformingPart()->component(0), level, flag);
                        cg_12.toMatrix(mat, src.getConformingPart()->component(2),
                                       dst.getConformingPart()->component(1), level, flag);
                        cg_20.toMatrix(mat, src.getConformingPart()->component(0),
                                       dst.getConformingPart()->component(2), level, flag);
                        cg_21.toMatrix(mat, src.getConformingPart()->component(1),
                                       dst.getConformingPart()->component(2), level, flag);
                        cg_22.toMatrix(mat, src.getConformingPart()->component(2),
                                       dst.getConformingPart()->component(2), level, flag);
                    }
...
// TODO: fix code duplication
            ...
            public:
                EGToP0DivOperatorNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage, uint_t minLevel,
                                           uint_t maxLevel)
                        : Operator<EGFunction<real_t>, P0Function<real_t> >(storage, minLevel, maxLevel),
                          p1x_to_p0(storage, minLevel, maxLevel), p1y_to_p0(storage, minLevel, maxLevel),
...
// TODO: fix code duplication
            ...
            public:
                P0ToEGDivTOperatorNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage, uint_t minLevel,
                                            uint_t maxLevel)
                        : Operator<P0Function<real_t>, EGFunction<real_t> >(storage, minLevel, maxLevel),
                          p0_to_p1x(storage, minLevel, maxLevel), p0_to_p1y(storage, minLevel, maxLevel),
            ...
            class EGP0StokesPreconditionerNitscheBC
                    : public Operator<EGP0StokesFunction<real_t>, EGP0StokesFunction<real_t> > {
            public:
                EGP0StokesPreconditionerNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage, uint_t minLevel,
                                                  uint_t maxLevel)
                        : Operator(storage, minLevel, maxLevel), viscOp(storage, minLevel, maxLevel),
            ...
            class EGP0StokesOperatorNitscheBC
                    : public Operator<EGP0StokesFunction<real_t>, EGP0StokesFunction<real_t> > {
            public:
                ...
                EGP0StokesOperatorNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage, size_t minLevel,
                                            size_t maxLevel)
                        : Operator(storage, minLevel, maxLevel), velocityBlockOp(storage, minLevel, maxLevel),
                          ...
                          energyNormOp(storage, minLevel, maxLevel), blockPrec(storage, minLevel, maxLevel) {}
            ...
            class EGP0EpsilonStokesPreconditionerNitscheBC
                    : public Operator<EGP0StokesFunction<real_t>, EGP0StokesFunction<real_t> > {
            public:
                EGP0EpsilonStokesPreconditionerNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage,
                                                         uint_t minLevel, uint_t maxLevel,
                                                         std::function<real_t(const Point3D &)> viscosity)
                        : Operator(storage, minLevel, maxLevel), viscOp(storage, minLevel, maxLevel, viscosity),
            ...
            class EGP0EpsilonOperatorStokesNitscheBC
                    : public Operator<EGP0StokesFunction<real_t>, EGP0StokesFunction<real_t> > {
            public:
                ...
                EGP0EpsilonOperatorStokesNitscheBC(const std::shared_ptr<PrimitiveStorage> &storage,
                        ...
                        : Operator(storage, minLevel, maxLevel),
                          velocityBlockOp(storage, minLevel, maxLevel, viscosity), div(storage, minLevel, maxLevel),
                          divT(storage, minLevel, maxLevel), energyNormOp(storage, minLevel, maxLevel, viscosity),
