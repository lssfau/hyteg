...
void P1toP1QuadraticProlongation::prolongateMacroFace( const real_t *src, real_t *dst, const uint_t & sourceLevel ) const
{
  ...
  for ( j = 2; j <= idx_t( N_c ) - 1; j += 2 )
  {
    ...
    //calculate offsets
    linearx  = real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i, j - 2, SD::VERTEX_C )] +
                                src[indexFromVertex( sourceLevel, i, j, SD::VERTEX_C )] );
    lineary  = real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i + 2, j - 2, SD::VERTEX_C )] +
                                src[indexFromVertex( sourceLevel, i, j - 2, SD::VERTEX_C )] );
    linearxy = real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i + 2, j - 2, SD::VERTEX_C )] +
                                 src[indexFromVertex( sourceLevel, i, j, SD::VERTEX_C )] );
    ...
    // left bottom corner
    dst[indexFromVertex( sourceLevel + 1, 2 * i + 1, 2 * j - 3, SD::VERTEX_C )] =
        real_c( 0.5 ) * ( linearx + lineary ) + real_c( 0.5 ) * offx + real_c( 0.5 ) * offy + real_c( 0.25 ) * offxy;
    // right bottom corner
    dst[indexFromVertex( sourceLevel + 1, 2 * i + 1, 2 * j - 2, SD::VERTEX_C )] =
        real_c( 0.5 ) * ( linearx + linearxy ) + real_c( 0.5 ) * offx + real_c( 0.25 ) * offy + real_c( 0.5 ) * offxy;
    // top corner
    dst[indexFromVertex( sourceLevel + 1, 2 * i + 2, 2 * j - 3, SD::VERTEX_C )] =
        real_c( 0.5 ) * ( linearxy + lineary ) + real_c( 0.25 ) * offx + real_c( 0.5 ) * offy + real_c( 0.5 ) * offxy;
  }
...
//calculate offsets
      lineary  = real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i - 2, j, SD::VERTEX_C )] +
                                  src[indexFromVertex( sourceLevel, i, j, SD::VERTEX_C )] );
      linearxy = real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i - 2, j, SD::VERTEX_C )] +
                                   src[indexFromVertex( sourceLevel, i, j - 2, SD::VERTEX_C )] );
      ...
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 1, 2 * j - 2, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( linearx + linearxy ) + real_c( 0.5 ) * offx + real_c( 0.25 ) * offy + real_c( 0.5 ) * offxy;
      // top corner
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 2, 2 * j - 1, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( linearxy + lineary ) + real_c( 0.25 ) * offx + real_c( 0.5 ) * offy + real_c( 0.5 ) * offxy;

      // boundary points
      // x-direction
      dst[indexFromVertex( sourceLevel + 1, 2 * i, 2 * j - 1, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( linearx + src[indexFromVertex( sourceLevel, i, j, SD::VERTEX_C )] ) + real_c( 0.75 ) * offx;
      dst[indexFromVertex( sourceLevel + 1, 2 * i, 2 * j - 3, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( linearx + src[indexFromVertex( sourceLevel, i, j - 2, SD::VERTEX_C )] ) + real_c( 0.75 ) * offx;
      //y-direction
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 1, 2 * j, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel,
                            ...
                            lineary ) +
          real_c( 0.75 ) * offy;
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 3, 2 * j, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel,
      ...
      //xy-direction
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 1, 2 * j - 3, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i, j - 2, SD::VERTEX_C )] + linearxy ) + real_c( 0.75 ) * offxy;
      dst[indexFromVertex( sourceLevel + 1, 2 * i - 3, 2 * j - 1, SD::VERTEX_C )] =
          real_c( 0.5 ) * ( src[indexFromVertex( sourceLevel, i - 2, j, SD::VERTEX_C )] + linearxy ) + real_c( 0.75 ) * offxy;
      // coarse points
      dst[indexFromVertex( sourceLevel + 1, 2 * i, 2 * j, SD::VERTEX_C )] =
          src[indexFromVertex( sourceLevel, i, j, SD::VERTEX_C )];
      dst[indexFromVertex( sourceLevel + 1, 2 * i, 2 * j - 2, SD::VERTEX_C )] =
