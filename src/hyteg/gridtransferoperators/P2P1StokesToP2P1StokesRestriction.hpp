...
{
 public:
   typedef P2toP2QuadraticInjection  VelocityRestriction_T;
   typedef P1toP1LinearRestriction<> PressureRestriction_T;

   P2P1StokesToP2P1StokesInjection( const std::shared_ptr< PrimitiveStorage >& storage, uint_t minLevel, uint_t maxLevel )
   : quadraticRestrictionOperator_( storage, minLevel, maxLevel )
   , projectMeanAfterRestriction_( true )
   {}
   // P2P1StokesToP2P1StokesInjection( bool projectMeanAfterRestriction )
   // : projectMeanAfterRestriction_( projectMeanAfterRestriction )
   // {}
 ...
 private:
   P2toP2QuadraticInjection  quadraticRestrictionOperator_;
   P1toP1LinearRestriction<> linearRestrictionOperator_;
...
NOTE: This restricts the FE function and calls the project function on it 
      so that the normal components are set to zero on the FreeslipBoundary
***************************************************************************/
class P2P1StokesToP2P1StokesInjectionWithFreeSlipProjection : public P2P1StokesToP2P1StokesInjection
{
 public:
   P2P1StokesToP2P1StokesInjectionWithFreeSlipProjection( const std::shared_ptr< PrimitiveStorage >& storage,
                                                          uint_t                                     minLevel,
                                                          uint_t                                     maxLevel,
                                                          std::shared_ptr< P2ProjectNormalOperator > projection )
   : P2P1StokesToP2P1StokesInjection( storage, minLevel, maxLevel )
   , projection_( projection )
   {}
   ...
   void restrict( const P2P1TaylorHoodFunction< real_t >& function,
                  ...
                  const DoFType&                          flag ) const override
   {
      P2P1StokesToP2P1StokesInjection::restrict( function, sourceLevel, flag );
      projection_->project( function, sourceLevel - 1, FreeslipBoundary );
      vertexdof::projectMean( function.p(), sourceLevel - 1 );
   }
