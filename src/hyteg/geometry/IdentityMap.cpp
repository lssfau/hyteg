...
namespace hyteg {
void IdentityMap::evalF(const Point3D &x, Point3D &Fx) const { Fx = x; }

void IdentityMap::evalFinv(const Point3D &xPhys, Point3D &xComp) const { xComp = xPhys; }
...
void IdentityMap::serializeSubClass(walberla::mpi::SendBuffer &sendBuffer) const { sendBuffer << Type::IDENTITY; }
