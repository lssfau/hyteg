...
void P2Form::integrateRow( const uint_t & row, const std::array< Point3D, 3 >& coords, Matrixr< 1, 6 >& elMat ) const
{
   switch ( row )
   {
   case 0:
      ...
      break;
   default:
      WALBERLA_ABORT( "P2Form::integrateRow() not implemented for row " << row );
   }
}

void P2Form::integrateRow( const uint_t & row, const std::array< Point3D, 4 >& coords, Matrixr< 1, 10 >& elMat ) const
{
   switch ( row )
   {
   case 0:
