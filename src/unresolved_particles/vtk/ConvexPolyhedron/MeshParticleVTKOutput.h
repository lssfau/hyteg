...
#include <unresolved_particles/vtk/ConvexPolyhedron/Types.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/data_sources/DataSourceAdapters.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/data_sources/FaceDataSource.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/data_sources/OutputSelectorFaceDataSource.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/data_sources/OutputSelectorVertexDataSource.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/data_sources/VertexDataSource.h>
#include <unresolved_particles/vtk/ConvexPolyhedron/tesselation/ConvexPolyhedronTesselation.h>
...
public:
   using ParticleSelectorFunc = std::function<bool (const walberla::unresolved_particles::data::ParticleStorage::iterator& pIt)>;

   typedef typename mesh::DistributedVTKMeshWriter<MeshType>::Vertices Vertices;

   MeshParticleVTKOutput( shared_ptr<walberla::unresolved_particles::data::ParticleStorage> ps,
   ...
   void operator()(AccessorWithShape_T & accessor) {
      static_assert(std::is_base_of<unresolved_particles::data::IAccessor, AccessorWithShape_T>::value, "Provide a valid accessor as template");
...
private:
   const shared_ptr<walberla::unresolved_particles::data::ParticleStorage> ps_;
   ...
   ParticleSelectorFunc particleSelector_ = [](const walberla::unresolved_particles::data::ParticleStorage::iterator& /*pIt*/){
      return true;
   };
};
...
void MeshParticleVTKOutput<MeshType>::assembleMesh(AccessorWithShape_T & accessor) {
   ...
   for (auto pIt = ps_->begin(); pIt != ps_->end(); ++pIt) {
      ...
      if (shape->getShapeType() == walberla::unresolved_particles::data::ConvexPolyhedron::SHAPE_TYPE) {
         const auto& convexPolyhedron = *static_cast<walberla::unresolved_particles::data::ConvexPolyhedron*>(shape);
