...
public:
   ...
   virtual void getData( const MeshType & mesh, const Vertices & vertices, std::vector<Type> & data,
                         ...
                         shared_ptr<walberla::unresolved_particles::data::ParticleStorage>) {
      for (const auto & vertex: vertices) {
         ...
         const Vector3<Type> d = walberla::unresolved_particles::getVelocityAtWFPoint(particleIdx, ac_, vertexPosition);
         data.push_back(d[0]);
         data.push_back(d[1]);
         data.push_back(d[2]);
      }
