...
struct ContainsPointFunctor
{
   ...
   bool operator()(const size_t particleIdx, const mesa_pd_convection::data::Sphere& sphere, const ParticleAccessor_T& ac, const Vector3<real_t>& point )
   {
   ...
   template<typename ParticleAccessor_T>
   bool operator()(const size_t particleIdx, const mesa_pd_convection::data::HalfSpace& halfSpace, const ParticleAccessor_T& ac, const Vector3<real_t>& point )
   {
      static_assert(std::is_base_of<mesa_pd_convection::data::IAccessor, ParticleAccessor_T>::value, "Provide a valid accessor as template");
