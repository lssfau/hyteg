...
class StokesConvergenceOrderTest
{
 ...
 private:
      ...
      void checkL2VeloRate( uint_t level )
      {
         ...
         WALBERLA_CHECK_GREATER_EQUAL( rates_[0],
                                       ...
                                       "Convergence L2 rate on level " << level << " too small (computed: " << rates_[0]
                                                                       << ", expected - eps: " << expectedL2VeloRate_ - 0.25
                                                                       << ")" );
      }
   ...
   ErrorArray RunStokesTestOnLevel( const uint_t& level )
   {
      ...
      {
      case 0: {
         MinResSolver< StokesOperatorType > solver( storage_, level, level, std::numeric_limits< uint_t >::max(), residualTol_ );
      ...
      default: {
         PETScMinResSolver< StokesOperatorType > solver(
             storage_, level, std::numeric_limits< PetscInt >::max(), residualTol_, residualTol_, numerator );
      ...
      {
         WALBERLA_CHECK_LESS( norms[0], discrErrorThreshhold_.second, "Threshhold for discretization error not reached." );
      }
