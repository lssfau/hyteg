...
namespace ufc
{
  ...
  public:
    ...
    virtual void evaluate(walberla::real_t * values,
                          const walberla::real_t * coordinates,
  ...
  public:
    ...
    virtual void evaluate_basis_all(walberla::real_t * values,
                                    const walberla::real_t * x,
                                    const walberla::real_t * coordinate_dofs,
                                    int cell_orientation) const = 0;
    ...
    virtual walberla::real_t evaluate_dof(std::size_t i,
                                const function& f,
                                const walberla::real_t * coordinate_dofs,
    ...
    virtual void evaluate_dofs(walberla::real_t * values,
                               const function& f,
                               const walberla::real_t * coordinate_dofs,
                               int cell_orientation,
                               const cell& c) const = 0;
    ...
    virtual void interpolate_vertex_values(walberla::real_t * vertex_values,
    ...
    virtual void tabulate_dof_coordinates(walberla::real_t * dof_coordinates,
                                          const walberla::real_t * coordinate_dofs) const = 0;
  ...
  public:
    ...
    virtual void compute_physical_coordinates(
        walberla::real_t * x, std::size_t num_points,
        const walberla::real_t * X,
        const walberla::real_t * coordinate_dofs) const = 0;
    ...
    virtual void compute_reference_coordinates(
        walberla::real_t * X, std::size_t num_points,
        const walberla::real_t * x,
        const walberla::real_t * coordinate_dofs, walberla::real_t cell_orientation) const = 0;
    ...
    //    walberla::real_t * X, walberla::real_t * J, walberla::real_t * detJ, walberla::real_t * K, std::size_t num_points,
    //    const walberla::real_t * x,
    //    const walberla::real_t * coordinate_dofs, walberla::real_t cell_orientation,
    //    std::size_t iterations, walberla::real_t tolerance) const = 0;

    /// Compute Jacobian of coordinate mapping J = dx/dX at reference coordinates X
    virtual void compute_jacobians(
        walberla::real_t * J, std::size_t num_points,
        const walberla::real_t * X,
        const walberla::real_t * coordinate_dofs) const = 0;

    /// Compute determinants of (pseudo-)Jacobians J
    virtual void compute_jacobian_determinants(
        walberla::real_t * detJ, std::size_t num_points,
        const walberla::real_t * J,
        walberla::real_t cell_orientation) const = 0;

    /// Compute (pseudo-)inverses K of (pseudo-)Jacobians J
    virtual void compute_jacobian_inverses(
        walberla::real_t * K, std::size_t num_points,
        const walberla::real_t * J, const walberla::real_t * detJ) const = 0;
  ...
  public:
    ...
    virtual void tabulate_tensor(walberla::real_t * A,
  ...
  public:
    ...
    virtual void tabulate_tensor(walberla::real_t * A,
                                 const walberla::real_t * const * w,
                                 const walberla::real_t * coordinate_dofs_0,
                                 const walberla::real_t * coordinate_dofs_1,
                                 std::size_t facet_0,
                                 std::size_t facet_1,
  ...
  public:
    ...
    virtual void tabulate_tensor(walberla::real_t * A,
                                 ...
                                 const walberla::real_t * quadrature_points,
                                 const walberla::real_t * quadrature_weights,
                                 const walberla::real_t * facet_normals,
                                 int cell_orientation) const = 0;
