...
void EdgeDoFPackInfo< ValueType >::packEdgeForVertex(const Edge *sender, const PrimitiveID &receiver, walberla::mpi::SendBuffer &buffer) const {
  ...
  if(vertexIdOnEdge == 0){
    ...
    buffer << edgeData[edgeIndexFromHorizontalEdge(level_, nbrEdges -1, stencilDirection::EDGE_HO_C)];
    buffer << edgeData[edgeIndexFromHorizontalEdge(level_, nbrEdges -1, stencilDirection::EDGE_DI_S)];
    if (sender->getNumNeighborFaces() == 2) {
      buffer << edgeData[edgeIndexFromHorizontalEdge(level_, nbrEdges -1, stencilDirection::EDGE_VE_NW)];
    }
  } else {
