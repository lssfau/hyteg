...
void MeshInfo::addEdge( const Edge & edge )
{
  ...
  {
    edges_[ sortedVertexIDs ] = Edge( sortedVertexIDs, edge.getDoFType(), edge.getBoundaryFlag() );
  }
}
...
void MeshInfo::addFace( const Face & face )
{
  ...
  {
    faces_[ sortedVertexIDs ] = Face( sortedVertexIDs, face.getDoFType(), face.getBoundaryFlag() );
  }
