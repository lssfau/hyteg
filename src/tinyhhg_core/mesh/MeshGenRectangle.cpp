                                  ...
                                  const meshFlavour flavour, uint_t nx, uint_t ny )
{
  ...
  auto splitCellCriss = [ rectMap ]( uint_t i, uint_t j, MeshInfo &meshInfo )
    {
      // lower triangle
      meshInfo.addFace( Face( { rectMap( i, j ), rectMap( i+1, j ), rectMap( i, j+1 ) }, Inner, 0 ) );

      // upper triangle
      meshInfo.addFace( Face( { rectMap( i+1, j ), rectMap( i+1, j+1 ), rectMap( i, j+1 ) }, Inner, 0 ) );
    };
  ...
  auto splitCellCross = [ rectMap ]( uint_t i, uint_t j, MeshInfo &meshInfo )
    {
      // lower triangle
      meshInfo.addFace( Face( { rectMap( i, j ), rectMap( i+1, j ), rectMap( i+1, j+1 ) }, Inner, 0 ) );

      // upper triangle
      meshInfo.addFace( Face( { rectMap( i, j ), rectMap( i+1, j+1 ), rectMap( i, j+1 ) }, Inner, 0 ) );
    };
      ...
      for ( uint_t j = 1; j < ny; ++j )
        {
          ...
          meshInfo.vertices_[id] = MeshInfo::Vertex( id, Point3D( node ), Inner, 0 );
        }
    }
  ...
  for ( uint_t i = 1; i < nx; ++i )
    {
      ...
      node[0] = llx + (real_t)i * hx;
      node[1] = lly;
      meshInfo.vertices_[id] = MeshInfo::Vertex( id, Point3D( node ), DirichletBoundary, 1 );
            ...
            for( uint_t j = 0; j < ny; ++j )
              {
                ...
                meshInfo.vertices_[idx] = MeshInfo::Vertex( idx, Point3D( { midx, midy, 0.0 } ), Inner, 0 );

                meshInfo.addFace( Face( { rectMap(  i ,  j  ), idx, rectMap( i+1,  j  ) }, Inner, 0 ) );
                meshInfo.addFace( Face( { rectMap( i+1,  j  ), idx, rectMap( i+1, j+1 ) }, Inner, 0 ) );
                meshInfo.addFace( Face( { rectMap( i+1, j+1 ), idx, rectMap(  i , j+1 ) }, Inner, 0 ) );
                meshInfo.addFace( Face( { rectMap(  i , j+1 ), idx, rectMap(  i ,  j  ) }, Inner, 0 ) );
      ...
      for ( i = j%2; i <= 2*nx - j%2; i += 2 )
        {
          ...
          node[0] = llx + (real_t)i * hx;
          node[1] = lly + (real_t)j * hy;
          meshInfo.vertices_[id] = MeshInfo::Vertex( id, Point3D( node ), dofType, boundaryFlag );
      ...
      for ( i = j; i < j + 2*len; i += 2 )
        {
          ...
          meshInfo.addFace( Face( { iNode[0], iNode[1], iNode[2] }, Inner, 0 ) );
  ...
  for ( const auto & it : faces )
    {
      ...
      this->addEdge( Edge( { fNode[0], fNode[1] }, edgeType, edgeBoundaryFlag ) );

      vertexA = verts.find( fNode[0] )->second.getCoordinates();
      vertexB = verts.find( fNode[2] )->second.getCoordinates();
      edgeType = edgeOnBoundary( vertexA, vertexB ) ? DirichletBoundary : Inner;
      edgeBoundaryFlag = edgeOnBoundary( vertexA, vertexB ) ? 1 : 0;
      this->addEdge( Edge( { fNode[0], fNode[2] }, edgeType, edgeBoundaryFlag ) );

      vertexA = verts.find( fNode[1] )->second.getCoordinates();
      vertexB = verts.find( fNode[2] )->second.getCoordinates();
      edgeType = edgeOnBoundary( vertexA, vertexB ) ? DirichletBoundary : Inner;
      edgeBoundaryFlag = edgeOnBoundary( vertexA, vertexB ) ? 1 : 0;
      this->addEdge( Edge( { fNode[1], fNode[2] }, edgeType, edgeBoundaryFlag ) );
    }
}
