...
private:
    ...
    struct { double *f1, *f2, *fac1, *fac2, *srt; } plm0_;
    ...
    void dplmbar( double *dplm, double *plm, int lmax );

    /// Determine position of value of Legendre function in internal array
    int getArrayIndex( uint_t deg, uint_t ord ) {
      int idx = 0;
      for( uint_t i = 0; i <= deg; ++i ) {
        for( uint_t j = 0; j <= i; ++j ) {
          idx++;
          if( i == deg && j == ord ) goto loopEnd;
        }
      }
      loopEnd:
...
public:
    ...
    SphericalHarmonicsTool( int lmax );
    ...
    double shconvert_eval(double *****sphdata,
    ...
    //! \param  y      y-coordinate of evaluation node
    //! \param  z      z-coordinate of evaluation node
    double shconvert_eval( int deg, int ord, double x, double y, double z );
    ...
    /// > Routine is stable in single and double precision to l,m = 511 at least;
    ...
    void plmbar( double *p, int lmax, double z );
    ...
    void dplmbar( double *dplm, int lmax, double z );
  ...
  /// -\varrho \end{pmatrix}
    ...
    /// \varrho = \sqrt{x^2 + y^2}
    /// \f]
    double evalVSH( int deg, int ord, double x, double y, double z, int ind,
                    int comp );
