...
real_t distanceToPlane( const Point3D & pointOfInterest, const Point3D & pointOnPlane, const Point3D & planeNormal )
{
...
Point3D tetrahedronInwardNormal( const Point3D & planeVertex0,
...
bool circleTriangleIntersection( const Point2D & centre, const real_t & radius, const Point2D & v1, const Point2D & v2, const Point2D & v3 )
{
  const auto v1x = v1[0];
  const auto v1y = v1[1];
  const auto v2x = v2[0];
...
bool sphereTriangleIntersection( const Point3D & centre, const real_t & radius, const Point3D & v1, const Point3D & v2, const Point3D & v3 )
{
  ...
  return circleTriangleIntersection( Point2D({ centrePlaneBasis[0], centrePlaneBasis[1] }), intersectionRadius,
...
bool isPointInTetrahedron( const Point3D & pointOfInterest,
                           ...
                           const Point3D & tetVertex3 )
{
  ...
  return distFace0 >= -real_c(1e-08)
    && distFace1 >= -real_c(1e-08)
    && distFace2 >= -real_c(1e-08)
    && distFace3 >= -real_c(1e-08);
}

/// Returns true if the passed sphere is completely located in the passed tetrahedron.
bool isSphereCompletelyInTetrahedron( const Point3D & sphereCenter,
                                      ...
                                      const Point3D & tetVertex3 )
{
  ...
  return distFace0 >= sphereRadius
         && distFace1 >= sphereRadius
         && distFace2 >= sphereRadius
         && distFace3 >= sphereRadius;
}

bool sphereTetrahedronIntersection( const Point3D & sphereCenter,
                                    const real_t  & sphereRadius,
                                    const Point3D & tetVertex0,
                                    const Point3D & tetVertex1,
                                    const Point3D & tetVertex2,
