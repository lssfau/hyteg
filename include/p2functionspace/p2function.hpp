#ifndef P2FUNCTION_HPP
#define P2FUNCTION_HPP
...
class P2Function : public Function
{
public:

  P2Function(const std::string& _name, Mesh& _mesh, size_t _minLevel, size_t _maxLevel)
    : Function(_name, _mesh, _minLevel, _maxLevel)
  {
    ...
    if (memory_id == -1)
    {
      fmt::printf("There was an error allocating P2 memory\n");
      std::exit(-1);
    }
    ...
    for (Vertex& vertex : mesh.vertices)
    {
      if (vertex.rank == rank)
      {
        P2Vertex::allocate(vertex, memory_id, minLevel, maxLevel);
      }
    }
    ...
    for (Edge& edge : mesh.edges)
    {
      if (edge.rank == rank)
      {
        P2Edge::allocate(edge, memory_id, minLevel, maxLevel);
      }
    }
    ...
    for (Face& face : mesh.faces)
    {
      if (face.rank == rank)
      {
        P2Face::allocate(face, memory_id, minLevel, maxLevel);
      }
    }
  }
  ...
  ~P2Function()
  {
    for (Vertex& vertex : mesh.vertices)
    {
      if (vertex.rank == rank)
      {
        P2Vertex::free(vertex, memory_id, minLevel, maxLevel);
      }
    }
    ...
    for (Edge& edge : mesh.edges)
    {
      if (edge.rank == rank)
      {
        P2Edge::free(edge, memory_id, minLevel, maxLevel);
      }
    }
    ...
    for (Face& face : mesh.faces)
    {
      if (face.rank == rank)
      {
        P2Face::free(face, memory_id, minLevel, maxLevel);
      }
    }
  }
