...
void testLaplace3D( const std::string& meshFile, const uint_t& level )
{
   ...
   for ( const auto& edgeIt : storage->getEdges() )
   {
      ...
      auto vertexToEdgeStencilMap =
          edge->getData( laplaceOperator3D.getVertexToEdgeOpr().getEdgeStencil3DID() )->getData( level );
      for ( uint_t neighborCellId = 0; neighborCellId < edge->getNumNeighborCells(); neighborCellId++ )
         for ( auto centerOrientation : vertexToEdgeStencilMap.at( neighborCellId ) )
            for ( auto direction : centerOrientation.second )
            {
   ...
   for ( const auto& vertexIt : storage->getVertices() )
   {
      ...
      // At vertices
      auto vertexToVertexStencilSize =
          vertex->getData( laplaceOperator3D.getVertexToVertexOpr().getVertexStencilID() )->getSize( level );
      auto vertexToVertexStencilArray =
          vertex->getData( laplaceOperator3D.getVertexToVertexOpr().getVertexStencilID() )->getPointer( level );
      ...
      auto   edgeToVertexStencilMap =
          vertex->getData( laplaceOperator3D.getEdgeToVertexOpr().getVertexStencil3DID() )->getData( level );
   ...
   std::function< real_t( const hyteg::Point3D& ) > linearInXY = []( const hyteg::Point3D& p ) -> real_t { return p[0] + p[1]; };
...
int main( int argc, char* argv[] )
{
   ...
   testLaplace3D( prependHyTeGMeshDir( "3D/regular_octahedron_8el.msh" ), 0 );
   testLaplace3D( prependHyTeGMeshDir( "3D/cube_24el.msh" ), 0 );

   testLaplace3D( prependHyTeGMeshDir( "3D/tet_1el.msh" ), 1 );
   testLaplace3D( prependHyTeGMeshDir( "3D/tet_1el.msh" ), 2 );
   testLaplace3D( prependHyTeGMeshDir( "3D/tet_1el.msh" ), 3 );
   testLaplace3D( prependHyTeGMeshDir( "3D/tet_1el.msh" ), 4 );
   testLaplace3D( prependHyTeGMeshDir( "3D/tet_tilted_1el.msh" ), 3 );

   testLaplace3D( prependHyTeGMeshDir( "3D/pyramid_2el.msh" ), 2 );
   testLaplace3D( prependHyTeGMeshDir( "3D/pyramid_2el.msh" ), 3 );

   testLaplace3D( prependHyTeGMeshDir( "3D/pyramid_4el.msh" ), 3 );
   testLaplace3D( prependHyTeGMeshDir( "3D/pyramid_tilted_4el.msh" ), 3 );
   testLaplace3D( prependHyTeGMeshDir( "3D/regular_octahedron_8el.msh" ), 3 );
