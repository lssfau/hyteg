...
static void testP2Restrict()
{
   ...
   auto x =
       std::make_shared< P2Function< real_t > >( "x", storage, sourceLevel - 1, sourceLevel, BoundaryCondition::create0123BC() );
   auto ident = P2Function< real_t >( "ident", storage, sourceLevel - 1, sourceLevel, BoundaryCondition::create0123BC() );
   std::function< real_t( const hyteg::Point3D& ) > one = []( const hyteg::Point3D& ) { return 1; };
   ...
   real_t expected = 5;
   expected += real_c( ( 1. / 8. ) * ( -46 + 3 * 47 + 3 * 48 - 49 ) );
   expected += real_c( ( 1. / 8. ) * ( -98 + 3 * 99 + 3 * 127 - 128 ) );
   expected += real_c( ( 1. / 8. ) * ( -70 - 72 ) );
   expected += real_c( ( 1. / 8. ) * ( -105 - 133 - 106 - 134 ) );

   WALBERLA_CHECK_FLOAT_EQUAL( storage->getEdge( PrimitiveID::create( 3 ) )
                                   ->getData( x->getVertexDoFFunction().getEdgeDataID() )
                                   ->getPointer( sourceLevel - 1 )[1],
                               expected );

   ///calculate expected entry for idx = 2 on edge 1
   expected = 21;
   expected += real_c( ( 1. / 8. ) * ( -64 + 3 * 65 + 3 * 66 - 67 ) );
   expected += real_c( ( 1. / 8. ) * ( -143 + 3 * 147 + 3 * 91 - 94 ) );
   expected += real_c( ( 1. / 8. ) * ( -115 - 122 ) );
   expected += real_c( ( 1. / 8. ) * ( -142 - 146 - 86 - 90 ) );

   WALBERLA_CHECK_FLOAT_EQUAL( storage->getEdge( PrimitiveID::create( 5 ) )
                                   ->getData( x->getVertexDoFFunction().getEdgeDataID() )
                                   ->getPointer( sourceLevel - 1 )[2],
                               expected );

   ///calculate expected entry for idx = 2 on edge 1
   expected = 16;
   expected += real_c( ( 1. / 8. ) * ( -61 + 3 * 60 + 3 * 59 - 58 ) );
   expected += real_c( ( 1. / 8. ) * ( -120 + 3 * 123 + 3 * 95 - 97 ) );
   expected += real_c( ( 1. / 8. ) * ( -148 - 153 ) );
   expected += real_c( ( 1. / 8. ) * ( -124 - 121 - 96 - 93 ) );

   WALBERLA_CHECK_FLOAT_EQUAL( storage->getEdge( PrimitiveID::create( 4 ) )
                                   ->getData( x->getVertexDoFFunction().getEdgeDataID() )
                                   ->getPointer( sourceLevel - 1 )[3],
                               expected );

   ///calculate expected entry for vertex dof on face at 4,2
   expected = 34;
   /// horizontal edges
   expected += real_c( ( 1. / 8. ) * ( -85 - 87 ) );
   expected += real_c( ( 1. / 8. ) * ( -79 + 3 * 80 + 3 * 81 - 82 ) );
   expected += real_c( ( 1. / 8. ) * ( -73 - 75 ) );
   /// vertical edges
   expected += real_c( ( 1. / 8. ) * ( -146 - 147 ) );
   expected += real_c( ( 1. / 8. ) * ( 3 * 142 - 143 ) );
   expected += real_c( ( 1. / 8. ) * ( -135 + 3 * 136 ) );
   expected += real_c( ( 1. / 8. ) * ( -129 - 130 ) );
   /// diagonal edges
   expected += real_c( ( 1. / 8. ) * ( -118 - 119 ) );
   expected += real_c( ( 1. / 8. ) * ( -113 + 3 * 114 ) );
   expected += real_c( ( 1. / 8. ) * ( 3 * 109 - 110 ) );
   expected += real_c( ( 1. / 8. ) * ( -102 - 103 ) );
   WALBERLA_CHECK_FLOAT_EQUAL(
       storage->getFace( PrimitiveID::create( 6 ) )
           ->getData( x->getVertexDoFFunction().getFaceDataID() )
           ->getPointer( sourceLevel -
                         1 )[hyteg::vertexdof::macroface::indexFromVertex( sourceLevel - 1, 2, 1, stencilDirection::VERTEX_C )],
       expected );
}
...
static void testP2Restrict2()
{
   ...
   std::shared_ptr< PrimitiveStorage > storage = std::make_shared< PrimitiveStorage >( *setupStorage );
   auto                                x =
       std::make_shared< P2Function< real_t > >( "x", storage, sourceLevel - 2, sourceLevel, BoundaryCondition::create0123BC() );
   ...
   real_t* edgeDoFCoarseData = storage->getFace( PrimitiveID::create( 6 ) )
                                   ->getData( x->getEdgeDoFFunction().getFaceDataID() )
                                   ->getPointer( sourceLevel - 1 );
   real_t* vertexDoFCoarseData = storage->getFace( PrimitiveID::create( 6 ) )
                                     ->getData( x->getVertexDoFFunction().getFaceDataID() )
                                     ->getPointer( sourceLevel - 1 );
   ...
   edgeDoFCoarseData = storage->getFace( PrimitiveID::create( 6 ) )
                           ->getData( x->getEdgeDoFFunction().getFaceDataID() )
                           ->getPointer( sourceLevel - 2 );
   vertexDoFCoarseData = storage->getFace( PrimitiveID::create( 6 ) )
                             ->getData( x->getVertexDoFFunction().getFaceDataID() )
                             ->getPointer( sourceLevel - 2 );
