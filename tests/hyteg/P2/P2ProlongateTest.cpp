...
static void testP2Prolongate()
{
   ...
   auto                                x =
       std::make_shared< P2Function< real_t > >( "x", storage, sourceLevel, sourceLevel + 2, BoundaryCondition::create0123BC() );
   ...
   real_t* edgeDoFFineData = storage->getFace( PrimitiveID::create( 6 ) )
                                 ->getData( x->getEdgeDoFFunction().getFaceDataID() )
                                 ->getPointer( sourceLevel + 1 );
   real_t* vertexDoFFineData = storage->getFace( PrimitiveID::create( 6 ) )
   ...
   for ( auto& edgeIT : storage->getEdges() )
   {
      ...
      edgeDoFFineData   = edge->getData( x->getEdgeDoFFunction().getEdgeDataID() )->getPointer( sourceLevel + 1 );
      vertexDoFFineData = edge->getData( x->getVertexDoFFunction().getEdgeDataID() )->getPointer( sourceLevel + 1 );
   ...
   edgeDoFFineData = storage->getFace( PrimitiveID::create( 6 ) )
                         ->getData( x->getEdgeDoFFunction().getFaceDataID() )
                         ->getPointer( sourceLevel + 2 );
   vertexDoFFineData = storage->getFace( PrimitiveID::create( 6 ) )
                           ->getData( x->getVertexDoFFunction().getFaceDataID() )
                           ->getPointer( sourceLevel + 2 );
...
static void testP2Prolongate2()
{
   ...
   std::shared_ptr< PrimitiveStorage > storage = std::make_shared< PrimitiveStorage >( *setupStorage );
   auto                                x =
       std::make_shared< P2Function< real_t > >( "x", storage, sourceLevel, sourceLevel + 1, BoundaryCondition::create0123BC() );
   typedef stencilDirection sD;
   ...
   for ( auto p : vertical )
   {
      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_VE_N )] = 16.0;

      prolongationOperator.prolongate( *x, sourceLevel, All );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_VE_N )],
                                  12. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_VE_S )],
                                  12. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_HO_E )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_HO_W )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_DI_NW )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_DI_SE )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_VE_SE )],
                                  4. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::EDGE_VE_NW )],
                                  4. );

      WALBERLA_CHECK_FLOAT_EQUAL( vertexDoFFineData[hyteg::vertexdof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2 + 1, sD::VERTEX_C )],
                                  16. );

      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_VE_N )] = 0.0;
   }
   /////////////////////////////
   ...
   for ( auto p : horizontal )
   {
      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_HO_E )] = 16.0;

      prolongationOperator.prolongate( *x, sourceLevel, All );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_VE_N )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_VE_S )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_HO_E )],
                                  12. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_HO_W )],
                                  12. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_DI_NW )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_DI_SE )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_HO_SE )],
                                  4. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::EDGE_HO_NW )],
                                  4. );

      WALBERLA_CHECK_FLOAT_EQUAL( vertexDoFFineData[hyteg::vertexdof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2, sD::VERTEX_C )],
                                  16. );

      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_HO_E )] = 0.0;
   }
   ///////////////////////////
   /// CHECK DIAGONAL EDGE ///
   ///////////////////////////
   for ( auto p : diagonal )
   {
      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_DI_NE )] = 16.0;

      prolongationOperator.prolongate( *x, sourceLevel, All );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_VE_N )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_VE_S )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_HO_E )],
                                  8. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_HO_W )],
                                  8. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_DI_NW )],
                                  12. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_DI_SE )],
                                  12. );

      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_DI_SW )],
                                  4. );
      WALBERLA_CHECK_FLOAT_EQUAL( edgeDoFFineData[hyteg::edgedof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::EDGE_DI_NE )],
                                  4. );

      WALBERLA_CHECK_FLOAT_EQUAL( vertexDoFFineData[hyteg::vertexdof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2 + 1, p.second * 2 + 1, sD::VERTEX_C )],
                                  16. );

      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getEdgeDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::edgedof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::EDGE_DI_NE )] = 0.0;
   }
   ////////////////////
   /// CHECK VERTEX ///
   ////////////////////
   for ( auto p : vertex )
   {
      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getVertexDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::vertexdof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::VERTEX_C )] = 16.0;

      prolongationOperator.prolongate( *x, sourceLevel, All );

      WALBERLA_CHECK_FLOAT_EQUAL( vertexDoFFineData[hyteg::vertexdof::macroface::indexFromVertex(
                                      sourceLevel + 1, p.first * 2, p.second * 2, sD::VERTEX_C )],
                                  16.,
                                  p.first << " " << p.second );
      ...
      storage->getFace( PrimitiveID::create( 6 ) )
          ->getData( x->getVertexDoFFunction().getFaceDataID() )
          ->getPointer( sourceLevel )[hyteg::vertexdof::macroface::indexFromVertex(
              sourceLevel, p.first, p.second, stencilDirection::VERTEX_C )] = 0.0;
   }
   ///////////////////////////
...
static void testP2InterpolateAndProlongate()
{
   ...
   std::function< real_t( const hyteg::Point3D& ) > exact = []( const hyteg::Point3D& xx ) {
      return 2. * xx[0] * xx[0] + 3. * xx[0] + 13. + 4. * xx[1] + 5. * xx[1] * xx[1];
   };
