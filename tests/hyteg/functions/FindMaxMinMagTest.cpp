...
void runFindTest( std::string                                       mesg,
                  findType                                          testType,
                  uint_t                                            theLevel,
                  funcType&                                         dofFunc,
                  std::function< real_t( const hyteg::Point3D& ) >& testFunc,
                  real_t                                            refVal,
                  DoFType                                           flag = All )
{
   real_t measure = 0.0;
...
int main( int argc, char* argv[] )
{
   ...
   runFindTest(
       "Test #8 (bc edge  ): max outer = ", FIND_MAX, theLevel, p1Func2D, testFuncMax, TEST_MAX_VALUE, DirichletBoundary );
   runFindTest( "                     max inner = ", FIND_MAX, theLevel, p1Func2D, testFuncMax, 0.0, Inner );
   runFindTest(
       "                     min outer = ", FIND_MIN, theLevel, p1Func2D, testFuncMin, TEST_MIN_VALUE, DirichletBoundary );
   runFindTest( "                     min inner = ", FIND_MIN, theLevel, p1Func2D, testFuncMin, 0.0, Inner );
   runFindTest(
       "                     mag outer = ", FIND_MAG, theLevel, p1Func2D, testFuncMin, TEST_MAG_VALUE, DirichletBoundary );
   ...
   runFindTest(
       "Test #9 (bc vertex): max outer = ", FIND_MAX, theLevel, p1Func2D, testFuncMax, TEST_MAX_VALUE, DirichletBoundary );
   runFindTest( "                     max inner = ", FIND_MAX, theLevel, p1Func2D, testFuncMax, 0.0, Inner );
   runFindTest(
       "                     min outer = ", FIND_MIN, theLevel, p1Func2D, testFuncMin, TEST_MIN_VALUE, DirichletBoundary );
   runFindTest( "                     min inner = ", FIND_MIN, theLevel, p1Func2D, testFuncMin, 0.0, Inner );
   ...
   runFindTest( "Test #A (Neumann  ): maximum   = ",
                FIND_MAX,
                theLevel,
                p1Func2D,
                testFuncCombo,
   ...
   meshInfo3D          = MeshInfo::fromGmshFile( prependHyTeGMeshDir( "3D/cube_6el.msh" ) );
