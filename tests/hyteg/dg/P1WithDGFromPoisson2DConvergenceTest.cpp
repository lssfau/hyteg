                ...
                bool                                      writeVTK = false )
{
   ...
   P1WithDGFormOperator A( storage, level, level, laplaceForm );
   P1WithDGFormOperator M( storage, level, level, massForm );

   DGToP1Operator< P1ToDG1InterpolationForm, real_t > opDGToP1Real ( storage, level, level, std::make_shared< P1ToDG1InterpolationForm >() );

   // Assemble RHS.
   DG1Function< real_t > fDG( "fDG", storage, level, level );
   fDG.evaluateLinearFunctional( rhsFunc, level );
   fDG.applyDirichletBoundaryConditions( laplaceForm, level );
   opDGToP1Real.apply(*fDG.getDGFunction(), f, level, All, Replace);

   // Interpolate solution
   sol.interpolate(solFunc, level, All);

   // Solve system.
   PETScCGSolver< P1WithDGFormOperator > solverA( storage, level, 1e-6, 1e-6, 10000 );
