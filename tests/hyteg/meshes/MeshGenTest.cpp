...
static void testMeshGenerator( testDomainType testDomain, MeshInfo::meshFlavour flavour )
{
   ...
   case THIN_SPHERICAL_SHELL:
      meshInfo = std::make_shared< MeshInfo >( MeshInfo::meshThinSphericalShell( 5, real_t( 2 ) ) );
      break;
   ...
   else if ( testDomain == THIN_SPHERICAL_SHELL )
   {
      WALBERLA_CHECK_EQUAL( numFaces, 320 );
      WALBERLA_CHECK_EQUAL( numEdges, 480 );
      WALBERLA_CHECK_EQUAL( numVerts, 162 );
   }
