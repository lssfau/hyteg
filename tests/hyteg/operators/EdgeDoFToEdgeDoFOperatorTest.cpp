...
static void testEdgeDoFToEdgeDoFOperator()
{
   ...
   for ( const auto& it : storage->getEdges() )
   {
      ...
      for ( const auto& idxIt : edgedof::macroedge::Iterator( maxLevel ) )
      {
         ...
         const real_t expectedValue = edgeSrcValue * ( macroEdgeHorizontalStencilValue +
                                                       real_c( edge->getNumNeighborFaces() ) *
                                                           ( macroEdgeDiagonalStencilValue + macroEdgeVerticalStencilValue ) );
   ...
   for ( const auto& it : storage->getFaces() )
   {
      ...
      for ( const auto& idxIt : edgedof::macroface::Iterator( maxLevel ) )
      {
         ...
         {
            const auto idx =
                edgedof::macroface::indexFromVerticalEdge( maxLevel, idxIt.x(), idxIt.y(), stencilDirection::EDGE_VE_C );

            const real_t expectedValue =
                edgeSrcValue *
                ( macroFaceVerticalStencilValue + 2.0 * ( macroFaceDiagonalStencilValue + macroFaceHorizontalStencilValue ) );
            WALBERLA_CHECK_FLOAT_EQUAL( ptr[idx], expectedValue );
         }
         ...
         {
            const auto idx =
                edgedof::macroface::indexFromHorizontalEdge( maxLevel, idxIt.x(), idxIt.y(), stencilDirection::EDGE_HO_C );

            const real_t expectedValue =
                edgeSrcValue *
                ( macroFaceHorizontalStencilValue + 2.0 * ( macroFaceDiagonalStencilValue + macroFaceVerticalStencilValue ) );
            WALBERLA_CHECK_FLOAT_EQUAL( ptr[idx], expectedValue );
         }
         ...
         {
            const auto idx =
                edgedof::macroface::indexFromDiagonalEdge( maxLevel, idxIt.x(), idxIt.y(), stencilDirection::EDGE_DI_C );

            const real_t expectedValue =
                edgeSrcValue *
                ( macroFaceDiagonalStencilValue + 2.0 * ( macroFaceHorizontalStencilValue + macroFaceVerticalStencilValue ) );
            WALBERLA_CHECK_FLOAT_EQUAL( ptr[idx], expectedValue );
         }
      }
   }
