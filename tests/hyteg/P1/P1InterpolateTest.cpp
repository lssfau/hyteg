...
int main( int argc, char** argv )
{
   ...
   MeshInfo                            meshInfo = MeshInfo::fromGmshFile( prependHyTeGMeshDir( "tri_1el.msh" ) );
   ...
   for ( auto faceIter : storage->getFaces() )
   {
      ...
      xStepSize =
          walberla::real_c( face->getCoordinates()[1][0] - face->getCoordinates()[0][0] ) / walberla::real_c( ( v_perEdge - 1 ) );
      yStepSize =
      ...
      for ( idx_t i = 0; i < idx_t( v_perEdge ); ++i )
      {
         for ( idx_t j = 0; j < idx_t( v_perEdge ) - i; ++j )
         {
            ...
            {
               WALBERLA_CHECK_FLOAT_EQUAL( face->getData( x.getFaceDataID() )->getPointer( maxLevel )[idx],
                                           0.0,
   ...
   for ( auto edgeIter : storage->getEdges() )
   {
      ...
      for ( uint_t i = 1; i < v_perEdge - 1; ++i )
      {
         WALBERLA_CHECK_FLOAT_EQUAL(
             edge->getData( x.getEdgeDataID() )->getPointer( maxLevel )[i], value, "i: " << i << " edge: " << *edge );
         value += 2 * xStepSize;
         value += yStepSize;
      }
   }
