...
void petscSolveTest( const uint_t& level, const std::string& meshFileName, const real_t& errEps, bool doVTKOutput = false )
{
   ...
   hyteg::P3Function< real_t > x( "x", storage, level, level );
   hyteg::P3Function< real_t > x_exact( "x_exact", storage, level, level );
   hyteg::P3Function< real_t > b( "b", storage, level, level );
   hyteg::P3Function< real_t > err( "err", storage, level, level );
   hyteg::P3Function< real_t > residuum( "residuum", storage, level, level );

   hyteg::P3ElementwiseDiffusionOperator A( storage, level, level );
   ...
   uint_t localDoFs  = hyteg::numberOfLocalDoFs< P3FunctionTag >( *storage, level );
   uint_t globalDoFs = hyteg::numberOfGlobalDoFs< P3FunctionTag >( *storage, level );
   ...
   if ( doVTKOutput )
   {
      VTKOutput vtkOutput( "../../output", "P3PetscSolve", storage );
      vtkOutput.add( x );
