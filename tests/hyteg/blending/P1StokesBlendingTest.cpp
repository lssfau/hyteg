...
int main( int argc, char* argv[] )
{
   ...
   auto smoother = std::make_shared< hyteg::UzawaSmoother< hyteg::P1BlendingStokesOperator > >(
       storage, minLevel, maxLevel, 0.3 );
   auto coarseGridSolver = std::make_shared< hyteg::MinResSolver< hyteg::P1BlendingStokesOperator > >( storage, minLevel, minLevel, coarseMaxiter );
   ...
   auto solver = hyteg::GeometricMultigridSolver< hyteg::P1BlendingStokesOperator >(
      storage, smoother, coarseGridSolver, restrictionOperator, prolongationOperator, minLevel, maxLevel, 2, 2, 2 );

   auto          start = walberla::timing::getWcTime();
   hyteg::P1BlendingStokesOperator L( storage, minLevel, maxLevel );
   ...
   std::function< real_t( const hyteg::Point3D& ) > exact_u = []( const hyteg::Point3D& x ) {
      return sin( x[0] ) * cos( x[1] ) / ( x[0] + 1 );
   };
   std::function< real_t( const hyteg::Point3D& ) > exact_v = []( const hyteg::Point3D& x ) {
      return -( ( x[0] + 1 ) * cos( x[0] ) - sin( x[0] ) ) * sin( x[1] ) / pow( x[0] + 1, 2 );
   };
   std::function< real_t( const hyteg::Point3D& ) > exact_p = []( const hyteg::Point3D& x ) {
      return pow( x[0], 2 ) * pow( x[1], 3 );
   };
   std::function< real_t( const hyteg::Point3D& ) > rhs_u = []( const hyteg::Point3D& x ) {
   ...
   std::function< real_t( const hyteg::Point3D& ) > rhs_v = []( const hyteg::Point3D& x ) {
   ...
   hyteg::communication::syncFunctionBetweenPrimitives( u.u, maxLevel );
   hyteg::communication::syncFunctionBetweenPrimitives( u.v, maxLevel );
   ...
   hyteg::communication::syncFunctionBetweenPrimitives( u_exact.v, maxLevel );
   ...
   //hyteg::communication::syncFunctionBetweenPrimitives( tmp.u, maxLevel );
   M.apply( tmp.u, f.u, maxLevel, hyteg::All );

   // Integrate RHS for v
   tmp.v.interpolate( rhs_v, maxLevel, hyteg::All );
   //hyteg::communication::syncFunctionBetweenPrimitives( tmp.v, maxLevel );
