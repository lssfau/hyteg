                     ...
                     const real_t& visc_matrix )
{
   ...
   std::function< real_t( const hyteg::Point3D& ) > exactU = [C_visc, r_inclusion]( const hyteg::Point3D& xx ) {
      return real_c( 2 ) * C_visc * xx[0];
   };
   std::function< real_t( const hyteg::Point3D& ) > exactV = [C_visc, r_inclusion]( const hyteg::Point3D& xx ) {
      return real_c( -2 ) * C_visc * xx[1];
   };
   std::function< real_t( const hyteg::Point3D& ) > exactP =
       [C_visc, r_inclusion, visc_inclusion, visc_matrix, rad]( const hyteg::Point3D& xx ) {
          if ( rad( xx ) <= r_inclusion )
             return real_c( 0 );
          else
             return real_c( 4 ) * C_visc * ( visc_inclusion - visc_matrix ) * std::pow( r_inclusion, 2.0 ) /
                    ( std::pow( xx[0], 2.0 ) + std::pow( xx[1], 2.0 ) ) * cos( 2 * atan2( xx[1], xx[0] ) );
       };
   ...
   real_t discr_l2_err_1_u =
       std::sqrt( err.uvw()[0].dotGlobal( err.uvw()[0], level, hyteg::Inner | hyteg::NeumannBoundary ) / (real_t) globalDoFs1 );
   real_t discr_l2_err_1_v =
       std::sqrt( err.uvw()[1].dotGlobal( err.uvw()[1], level, hyteg::Inner | hyteg::NeumannBoundary ) / (real_t) globalDoFs1 );
   real_t discr_l2_err_1_p =
       std::sqrt( err.p().dotGlobal( err.p(), level, hyteg::Inner | hyteg::NeumannBoundary ) / (real_t) globalDoFs1 );
   ...
   /*
  typedef hyteg::StokesPressureBlockPreconditioner< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator, hyteg::P1LumpedInvMassOperator >
        PressurePreconditioner_T;
   auto pressurePrec = std::make_shared< PressurePreconditioner_T >( storage, 0, level );

   auto gaussSeidel = std::make_shared< hyteg::GaussSeidelSmoother< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator::VelocityOperator_T > >();
   auto uzawaVelocityPreconditioner = std::make_shared< hyteg::StokesVelocityBlockBlockDiagonalPreconditioner< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator > >( storage, gaussSeidel );
   auto smoother =
       std::make_shared< hyteg::UzawaSmoother< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator > >( storage, uzawaVelocityPreconditioner, 0, level, 0.4 );
   ...
   auto coarseGridSolver = std::make_shared< hyteg::PETScLUSolver< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator > >( storage, 0 );
   hyteg::GeometricMultigridSolver< hyteg::P2P1ElementwiseAffineEpsilonStokesOperator > GMG(
       storage, smoother, coarseGridSolver, restriction, prolongation, 0, level, 3, 3, 0 );
*/
...
int main( int argc, char* argv[] )
{
   ...
   SolViBenchmark( 1, 2, 10, 0.2, 1000, 1 );
