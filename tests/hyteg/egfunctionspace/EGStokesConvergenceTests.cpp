            ...
            public:
                ...
                StokesConvergenceOrderTest(const std::string &testName,
                    ...
                    WALBERLA_LOG_INFO_ON_ROOT(
                            walberla::format("%6s|%15s|%15s|%15s|%15s", "level", "error_v", "error_p", "rate_v",
                                             "rate_p"));
                    ...
                    for (uint_t level = minLevel; level <= maxLevel; level++) {
                        ...
                        auto error_per_h = RunStokesTestOnLevel(testName, level, solTuple, rhsTuple, Op, storage,
                                                                solverType, writeVTK);
                        ...
                        WALBERLA_LOG_INFO_ON_ROOT(walberla::format(
                                "%6d|%15.2e|%15.2e|%15.2e|%15.2e", level, currentError_v, currentError_p, currentRate_v,
                                currentRate_p));
                    }
                ...
                std::tuple<real_t, real_t, real_t> RunStokesTestOnLevel(const std::string &testName,
                    ...
                    WALBERLA_LOG_INFO_ON_ROOT("Global DoFs: " << numberOfGlobalDoFs(numerator, level) << ", u DoFs: " << numberOfGlobalDoFs(numerator.uvw(), level) << ", p DoFs: " << numberOfGlobalDoFs(numerator.p(), level));

                    // solution, rhs as a lambda function
                    auto [u_x_expr, u_y_expr, u_z_expr, p_expr] = solTuple;
                    auto [f_x_expr, f_y_expr, f_z_expr, g_expr] = rhsTuple;
                    ...
                    if constexpr (isP2P1Discr<StokesOperatorType>()) {
                        ...
                        if constexpr (isEGP0Discr<StokesOperatorType>()) {
                            ...
                            if (!storage->hasGlobalCells()) {
                                ...
                                rhs.uvw().interpolate({u_x_expr, u_y_expr, u_z_expr}, level, DirichletBoundary);
                                WALBERLA_LOG_INFO_ON_ROOT(
                                        "Pressure mass not implemented in 3D, careful with velocity fields with divergence!")
                            }
                        } else {
                    ...
                    if constexpr (isEGP0Discr<StokesOperatorType>()) {
                        WALBERLA_LOG_INFO_ON_ROOT(
                                "||e_v_disc|| = " << sqrt(err.uvw().getDiscontinuousPart()->dotGlobal(
                                        *err.uvw().getDiscontinuousPart(), level, Inner) /
                                                          real_c(numberOfGlobalDoFs(*err.uvw().getDiscontinuousPart(),
                                                                                    level)))
                                                  << ", ||e_v_conf|| = "
                                                  << sqrt(err.uvw().getConformingPart()->dotGlobal(
                                                          *err.uvw().getConformingPart(), level, Inner) /
