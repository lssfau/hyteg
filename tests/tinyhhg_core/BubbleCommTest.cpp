...
//////////////////// BLUE CELL //////////////////////
  ...
  for (auto &edgeIt : storage->getEdges()) {
    ...
    for(uint_t i = 0; i < nbrVertices.size(); ++i)
    {
      ...
      if(vertexIdOnEdge == 0){
        WALBERLA_CHECK_EQUAL(edgeData[BubbleEdge::indexFaceFromVertex(maxLevel, 0, stencilDirection::CELL_GRAY_SE )],
                             vertexData[vertex->face_index(edge.neighborFaces()[0])]);
        numberOfChecks++;
        if(edge.getNumHigherDimNeighbors() == 2)
        {
          WALBERLA_CHECK_EQUAL(edgeData[BubbleEdge::indexFaceFromVertex(maxLevel, 0, stencilDirection::CELL_GRAY_NE)],
                               vertexData[vertex->face_index(edge.neighborFaces()[1])]);
          numberOfChecks++;
        }
      } else if( vertexIdOnEdge == 1){
        WALBERLA_CHECK_EQUAL(edgeData[BubbleEdge::indexFaceFromVertex(maxLevel, vPerEdge - 1, stencilDirection::CELL_GRAY_SW )],
                             vertexData[vertex->face_index(edge.neighborFaces()[0])]);
        numberOfChecks++;
        if(edge.getNumHigherDimNeighbors() == 2)
        {
          WALBERLA_CHECK_EQUAL(edgeData[BubbleEdge::indexFaceFromVertex(maxLevel, vPerEdge - 1, stencilDirection::CELL_GRAY_NW)],
                               vertexData[vertex->face_index(edge.neighborFaces()[1])]);
          numberOfChecks++;
        }
      } else {
