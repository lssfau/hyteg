...
void testBlockForestCoupling()
{
   ...
   unresolved_particles::mpi::SyncNextNeighborsNoGhosts SNN;
...
void testSetupPrimitiveStorageCoupling2D()
{
   ...
   PrimitiveStorageUnresolvedParticlesInterface domain( storage );
   ...
   for ( auto p : particleStorage )
   {
      auto isGhost = p.getFlags().getRawDataRef() == walberla::unresolved_particles::data::particle_flags::GHOST;
