...
const static std::map< std::string, DiscretizationType > strToDiscretizationType = { { "P1P1", P1P1 },
...
class PetscSolver : public Solver< Operator_T >
{
 ...
 public:
   ...
   : storage_( storage )
   , velocityUBC_( velocityUBC )
   , velocityVBC_( velocityVBC )
   {
   ...
   void solve( const Operator_T& A, const Function_T< real_t >& x, const Function_T< real_t >& b, size_t level )
   {
 ...
 public:
   ...
   void solve( const Operator_T&, const Function_T< real_t >&, const Function_T< real_t >&, uint_t ) override
   {
...
void setRightBFSBoundaryNeumannBFS( SetupPrimitiveStorage& setupStorage )
{
   ...
   for ( const auto& it : setupStorage.getVertices() )
   {
      if ( std::fabs( it.second->getCoordinates()[0] - 2.0 ) < eps && it.second->getCoordinates()[1] > eps &&
           it.second->getCoordinates()[1] < 1.0 - eps )
      {
         setupStorage.setMeshBoundaryFlag( it.first, 2 );
      }
...
template < template < typename ValueType > class StokesFunction_T,
           class StokesOperator_T,
           class RestrictionOperator_T,
           class ProlongationOperator_T,
           class VelocityMassMatrix_T >
void run( const MeshInfo&                                       meshInfo,
          const uint_t&                                         minLevel,
          const uint_t&                                         maxLevel,
          const SolverType&                                     solverType,
          const SolverType&                                     coarseGridSolver,
          const uint_t&                                         numMGCycles,
          const uint_t&                                         preSmooth,
          const uint_t&                                         postSmooth,
          const uint_t&                                         incrementSmooth,
          const real_t&                                         uzawaRelaxParam,
          const real_t                                          targetResidual,
          ...
          const bool&                                           printTimer )
{
   ...
   real_t currentResidualL2 = std::sqrt( r.dotGlobal( r, maxLevel, hyteg::Inner | hyteg::NeumannBoundary ) ) /
   ...
   if ( compareWithAnalyticalSolution )
   {
      ...
      const real_t currentErrorL2 =
          std::sqrt( error.dotGlobal( error, maxLevel, hyteg::All ) ) /
          real_c( hyteg::numberOfGlobalDoFs< typename StokesFunction_T< real_t >::Tag >( *storage, maxLevel ) );
   ...
   case PETSC: {
      ...
      if ( rescalePressure )
      {
         const real_t minPressure = u.p().getMinDoFValue( maxLevel, DoFType::All );
   ...
   case UZAWA: {
      ...
      case EMPTY: {
         ...
         auto uzawaVelocityPreconditioner =
             std::make_shared< hyteg::StokesVelocityBlockBlockDiagonalPreconditioner< StokesOperator_T > >( storage,
                                                                                                            gaussSeidel );
      ...
      case PETSC: {
         ...
         for ( uint_t mgIteration = 0; mgIteration < numMGCycles; mgIteration++ )
         {
            ...
            WALBERLA_LOG_INFO_ON_ROOT( "[StokesFlowSolverComparison] "
                                       ...
                                       << currentResidualL2 << " | " << std::setw( 16 ) << std::scientific
                                       << currentResidualL2 / lastResidualL2 << " | " << std::setw( 7 ) << std::fixed
                                       << std::setprecision( 3 ) << timer.last() << "s" )
         }
         vtkOutput.write( maxLevel, numMGCycles );
         break;
      }
...
int main( int argc, char* argv[] )
{
   ...
   if ( args.size() <= 1 )
   {
      WALBERLA_LOG_INFO_ON_ROOT( "[StokesFlowSolverComparison] No parameter file given, falling back to "
   ...
   const hyteg::MeshInfo meshInfo = [meshType, squareDomainSolutionType]() {
      if ( meshType == "square" )
      {
         return hyteg::MeshInfo::meshRectangle( squareDomainSolutionType == "colliding_flow" ||
                                                        squareDomainSolutionType == "poiseuille_flow" ?
                                                    hyteg::Point2D( -1, -1 ) :
                                                    hyteg::Point2D( 0, 0 ),
                                                hyteg::Point2D( 1, 1 ),
   ...
   const auto setMeshBoundaryFlags = [meshType,
                                      squareDomainSolutionType]() -> std::function< void( hyteg::SetupPrimitiveStorage& ) > {
   ...
   const auto setUVelocityBC = [meshType, zero, squareDomainSolutionType]() -> std::function< real_t( const hyteg::Point3D& ) > {
      ...
      else if ( meshType == "square" )
      {
         ...
         {
            return []( const hyteg::Point3D& x ) -> real_t { return real_c( 20 ) * x[0] * x[1] * x[1] * x[1]; };
         }
   ...
   const auto setVVelocityBC = [meshType, zero, squareDomainSolutionType]() -> std::function< real_t( const hyteg::Point3D& ) > {
      if ( meshType == "porous_coarse" || meshType == "porous_fine" || meshType == "bfs_coarse" || meshType == "bfs_fine" )
      {
         return zero;
      }
      else if ( meshType == "square" )
   ...
   const auto setUVelocityRHS = [meshType, zero, squareDomainSolutionType]() -> std::function< real_t( const hyteg::Point3D& ) > {
      ...
      else if ( meshType == "square" )
      {
         ...
         {
            return []( const hyteg::Point3D& x ) -> real_t {
               return real_c( 2 ) * std::pow( walberla::math::pi, 2 ) * std::sin( walberla::math::pi * x[0] ) *
                          std::sin( walberla::math::pi * x[1] ) +
   ...
   const auto setVVelocityRHS = [meshType, zero, squareDomainSolutionType]() -> std::function< real_t( const hyteg::Point3D& ) > {
      ...
      else if ( meshType == "square" )
      {
         ...
         {
            return []( const hyteg::Point3D& x ) -> real_t {
               return real_c( 2 ) * std::pow( walberla::math::pi, 2 ) * std::cos( walberla::math::pi * x[0] ) *
                          std::cos( walberla::math::pi * x[1] ) +
   ...
   const auto solutionU =
       [meshType, squareDomainSolutionType, setUVelocityBC]() -> std::function< real_t( const hyteg::Point3D& ) > {
      if ( meshType == "square" && squareDomainSolutionType == "poiseuille_flow" )
      {
         return []( const hyteg::Point3D& x ) -> real_t { return real_c( 1 - x[1] * x[1] ); };
      }
      else
      {
   ...
   const auto solutionP =
       [meshType, zero, squareDomainSolutionType, rescalePressure]() -> std::function< real_t( const hyteg::Point3D& ) > {
      if ( meshType == "porous_coarse" || meshType == "porous_fine" || meshType == "bfs_coarse" || meshType == "bfs_fine" )
      {
         return zero;
      }
      else if ( meshType == "square" )
      {
         if ( squareDomainSolutionType == "colliding_flow" )
         {
            return [rescalePressure]( const hyteg::Point3D& x ) -> real_t {
               return real_c( 60 ) * x[0] * x[0] * x[1] - real_c( 20 ) * x[1] * x[1] * x[1] +
                      ( rescalePressure ? real_c( 40 ) : real_c( 0 ) );
            };
         }
         else if ( squareDomainSolutionType == "poiseuille_flow" )
         {
            return []( const hyteg::Point3D& x ) -> real_t { return real_c( -2.0 * x[0] ); };
         }
         else
         {
            return []( const hyteg::Point3D& x ) -> real_t {
   ...
   parameterOverview << "[StokesFlowSolverComparison] Parameter overview:\n"
                     ...
                     << "\n"
                        "                             - discretization:              "
                     << discretizationTypeString
                     << "\n"
                        "                             - solver:                      "
                     << solverTypeString
                     << "\n"
                        "                             - analytical solution:         "
                     << ( compareWithAnalyticalSolution ? "available" : "not available" )
                     << "\n"
                        "                             - solution type:               "
                     << ( compareWithAnalyticalSolution ? squareDomainSolutionType : "-" )
                     << "\n"
                        "                             - timer output:                "
                     << ( printTimer ? "enabled" : "disabled" )
                     << "\n"
                        "                             - rescale pressure:            "
                     << ( rescalePressure ? "enabled" : "disabled" ) << "\n";
   ...
   case hyteg::P1P1:
      hyteg::run< hyteg::P1StokesFunction,
                  ...
                  hyteg::P1ConstantMassOperator >( meshInfo,
                                                   minLevel,
                                                   maxLevel,
                                                   solverType,
                                                   coarseGridSolver,
   ...
   case hyteg::TaylorHood:
      hyteg::run< hyteg::P2P1TaylorHoodFunction,
                  ...
                  hyteg::P2ConstantMassOperator >( meshInfo,
                                                   minLevel,
                                                   maxLevel,
                                                   solverType,
                                                   coarseGridSolver,
