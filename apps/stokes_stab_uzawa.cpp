...
int main( int argc, char* argv[] )
{
   ...
   auto coarseGridSolver =
       std::make_shared< hyteg::MinResSolver< hyteg::P1P1StokesOperator > >( storage, minLevel, minLevel, coarseMaxiter );
   ...
   for ( outer = 0; outer < maxOuterIter; ++outer )
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT( walberla::format(
          "%6d|%10.3e|%10.3e|%10.3e|%10.3e", outer + 1, abs_res, rel_res, abs_res / abs_res_old, end - start ) );
      totalTime += end - start;
