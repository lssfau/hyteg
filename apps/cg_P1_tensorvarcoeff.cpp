...
int main(int argc, char* argv[])
{
  ...
  std::function<real_t(const hhg::Point3D&)> coeff_11 = [](const hhg::Point3D& x) { return 7*pow(x[0], 2) + 3*x[0] + 4*x[1] + 1; };
  std::function<real_t(const hhg::Point3D&)> coeff_12 = [](const hhg::Point3D& x) { return 4*pow(x[0], 2) + 2*x[0] + 3*x[1] + 1; };
  std::function<real_t(const hhg::Point3D&)> coeff_22 = [](const hhg::Point3D& x) { return 8*pow(x[0], 2) + 4*x[0] + 5*x[1] + 1; };
  std::function<real_t(const hhg::Point3D&)> exact = [](const hhg::Point3D& x) { return sin(x[0])*sinh(x[1]); };
  std::function<real_t(const hhg::Point3D&)> rhs = [](const hhg::Point3D& x) { return -2*(4*x[0] + 1)*sin(x[0])*cosh(x[1]) - (14*x[0] + 3)*cos(x[0])*sinh(x[1]) - 2*(4*pow(x[0], 2) + 2*x[0] + 3*x[1] + 1)*cos(x[0])*cosh(x[1]) + (7*pow(x[0], 2) + 3*x[0] + 4*x[1] + 1)*sin(x[0])*sinh(x[1]) - (8*pow(x[0], 2) + 4*x[0] + 5*x[1] + 1)*sin(x[0])*sinh(x[1]) - 5*sin(x[0])*cosh(x[1]) - 3*cos(x[0])*sinh(x[1]); };
