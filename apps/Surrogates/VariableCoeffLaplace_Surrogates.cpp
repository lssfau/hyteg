...
struct OperatorHandler
{
   ...
   template < class OP >
   static double setup( std::shared_ptr< OP >, const uint_t )
   {
      return 0;
   }
};
...
void solveTmpl( std::shared_ptr< PrimitiveStorage > storage,
                ...
                c_function&                         boundary,
                c_function&                         rhs,
                const uint_t                        minPolyDegree = 0,
                const uint_t                        maxPolyDegree = 0 )
{
   ...
   // define solver
   auto coarseGridSolver = std::make_shared< hyteg::CGSolver< typename FE::divKgrad > >(
   ...
   GeometricMultigridSolver< typename FE::divKgrad > GMGSolver(
...
void solve( const StencilType                   T,
            ...
            c_function&                         boundary,
            c_function&                         rhs,
            const uint_t                        minPolyDegree,
            const uint_t                        maxPolyDegree )
{
   switch ( T )
   {
   case VARIABLE:
      WALBERLA_LOG_INFO_ON_ROOT( "Operatortype: Variable Stencil" );
      solveTmpl< P, VARIABLE >(
          storage, minLevel, maxLevel, max_outer_iter, max_cg_iter, mg_tolerance, coarse_tolerance, vtk, exact, boundary, rhs );
      break;
...
void showStencilFunction( std::shared_ptr< PrimitiveStorage > storage, const uint_t level, const real_t alpha )
{
   ...
   for ( auto& faceit : storage->getFaces() )
   {
      ...
      for ( const auto& it : hyteg::edgedof::macroface::Iterator( level, 0 ) )
      {
         ...
         P2::variablestencil::macroface::assembleStencil(
             form, x, dirS, dirSE, dirE, dirN, dirNW, dirW, dirNE, VtVStencil, EtVStencil, VtEStencil, EtEStencil );
         idx_t i = it.x();
         idx_t j = it.y();
