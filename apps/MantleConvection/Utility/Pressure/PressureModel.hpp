...
class PressureModel
{
 public:
   ...
   PressureModel( std::string prefix = "" )
   : prefix_( prefix )
   {}
   ...
   {
      WALBERLA_UNUSED( x );
      WALBERLA_ABORT( "evaluate not implemented for PressureModel abstract base class!" );
      return ValueType();
   }

   virtual std::function< real_t( const hyteg::Point3D& ) > getPressureFct()
   {
   ...
   virtual std::ostream& print( std::ostream& os, uint_t offset = 0 ) const
   {
      ...
      WALBERLA_ABORT( "print not implemented for PressureModel abstract base class!" );
      return os;
   }
   ...
   {
      f.interpolate( getPressureFct(), level, flag );
   }
