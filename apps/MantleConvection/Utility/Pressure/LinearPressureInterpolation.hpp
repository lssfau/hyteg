...
class LinearPressureInterpolation : public PressureModel< real_t >
{
 public:
   using PressureModel< real_t >::prefix_;

   LinearPressureInterpolation( NondimensionalisationParameters& nondimensionalisation,
                                walberla::Config::BlockHandle&   parameters,
                                std::vector< real_t >&           range,
                                std::vector< real_t >&           values,
                                std::string                      prefix = "" )
   : PressureModel< real_t >( prefix )
   , range_( range )
   , values_( values )
   {
      ...
      {
         WALBERLA_LOG_WARNING( "The values and range vectors of LinearPressureInterpolation should be of the same size!" )
      }
   }
   ...
   std::ostream& print( std::ostream& os, uint_t offset = 0 ) const override
   {
      ...
      os << std::string( offset, ' ') << "########### Linear Pressure Interpolation ###########"             << "\n";
      os << std::string( offset, ' ') << "#####################################################"             << "\n";
...
inline std::ostream& operator<<( std::ostream& os, const LinearPressureInterpolation& lpi )
{
   return lpi.print( os );
}
} // namespace MantleConvection