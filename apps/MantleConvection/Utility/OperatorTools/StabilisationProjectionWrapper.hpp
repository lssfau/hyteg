...
class StabilisationProjectionWrapper : public hyteg::Operator< typename OperatorType::srcType, typename OperatorType::dstType >,
                                       public hyteg::OperatorWithInverseDiagonal< typename OperatorType::srcType >
{
 public:
   StabilisationProjectionWrapper( std::shared_ptr< OperatorType > op, bool lowMemoryMode = false )
      ...
      if constexpr ( useTmpSrc )
      {
         if ( !lowMemoryMode_ )
         {
            tmpSrc_ = std::make_shared< typename OperatorType::srcType >(
                "StabilisationProjectionWrapper tmpSrc", op->getStorage(), op->getMinLevel(), op->getMaxLevel() );
         }
      }
      if constexpr ( useTmpDst )
      {
         if ( !lowMemoryMode_ )
         {
            tmpDst_ = std::make_shared< typename OperatorType::dstType >(
                "StabilisationProjectionWrapper tmpDst", op->getStorage(), op->getMinLevel(), op->getMaxLevel() );
         }
      }
   }
