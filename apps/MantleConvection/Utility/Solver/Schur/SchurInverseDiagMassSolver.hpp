...
class SchurInverseDiagMassSolver : public SchurSolver< SchurOperator< typename MassOperatorType::dstType > >
{
 public:
   using SchurSolver< SchurOperator< typename MassOperatorType::dstType > >::prefix_;

   SchurInverseDiagMassSolver( walberla::Config::BlockHandle&                    parameters,
   ...
   : SchurSolver< SchurOperator< typename MassOperatorType::dstType > >( prefix )
   {
      ...
      if ( computeInverse )
      {
         MassOperator.computeInverseDiagonalOperatorValues();
      }

      funcMult_ =
          std::make_shared< hyteg::FunctionMultiplicationPreconditioner< SchurOperator< typename MassOperatorType::dstType > > >(
              storage, minLevel, maxLevel, MassOperator.getInverseDiagonalValues() );
   }
   ...
   std::ostream& print( std::ostream& os, uint_t offset = 0 ) const override
   {
      ...
      os << std::string( offset, ' ') << "######## Schur Inverse Diag Mass Solver ########"                                                       << "\n";
      os << std::string( offset, ' ') << "################################################"                                                       << "\n";
...
template < class MassOperatorType >
inline std::ostream& operator<<( std::ostream& os, const SchurInverseDiagMassSolver< MassOperatorType >& sidm )
{
   return sidm.print( os );
}
