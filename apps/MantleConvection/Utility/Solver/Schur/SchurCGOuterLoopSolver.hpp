...
class SchurCGOuterLoopSolver : public SchurSolver< SchurOperator< typename OperatorType::srcType::PressureFunction_T > >
{
 public:
   ...
   SchurCGOuterLoopSolver( walberla::Config::BlockHandle&                                                 parameters,
                           ...
                           std::string                                           prefix                   = "" )
   : SchurCGOuterLoopSolver( parameters,
                             storage,
                             minLevel,
                             maxLevel,
                             saddleOp,
   ...
   , lowMemoryMode_( lowMemoryMode )
   {
      ...
      maxIterations_     = parameters.getParameter< uint_t >( prefix + std::string( "SchurCGOuterLoopMaxIterations" ) );
      relativeTolerance_ = parameters.getParameter< real_t >( prefix + std::string( "SchurCGOuterLoopRelativeTolerance" ) );
      absoluteTolerance_ = parameters.getParameter< real_t >( prefix + std::string( "SchurCGOuterLoopAbsoluteTolerance" ) );
      printInfo_         = parameters.getParameter< bool >( prefix + std::string( "SchurCGOuterLoopPrintInfo" ) );
      ...
      SchurCGOuterLoopSolver_ = std::make_shared< hyteg::CGSolver< ApproximateSchurOperatorType > >(
          storage, minLevel, maxLevel, maxIterations_, relativeTolerance_, absoluteTolerance_, substSolver_, lowMemoryMode_ );
      SchurCGOuterLoopSolver_->setPrintInfo( printInfo_ );

      if ( prefix_ != "" )
      {
         SchurCGOuterLoopSolver_->setName( std::string( "CG Schur Outer Loop prefix " ) + prefix_ );
      }
      else
      {
         SchurCGOuterLoopSolver_->setName( "CG Schur Outer Loop" );
      }
   }
   ...
   {
      SchurCGOuterLoopSolver_->solve( *approximateSchurOperator_, x, b, level );
   };

   std::shared_ptr< hyteg::CGSolver< ApproximateSchurOperatorType > > getSolver() { return SchurCGOuterLoopSolver_; }

   std::ostream& print( std::ostream& os, uint_t offset = 0 ) const override
   {
      // clang-format off
      os << std::string( offset, ' ') << "################################################"                                                      << "\n";
      os << std::string( offset, ' ') << "########## Schur CG Outer Loop Solver ##########"                                                      << "\n";
      os << std::string( offset, ' ') << "################################################"                                                      << "\n";
 ...
 private:
   ...
   std::shared_ptr< hyteg::CGSolver< ApproximateSchurOperatorType > >      SchurCGOuterLoopSolver_;
...
template < class OperatorType >
inline std::ostream& operator<<( std::ostream& os, const SchurCGOuterLoopSolver< OperatorType >& scgols )
{
   return scgols.print( os );
}
