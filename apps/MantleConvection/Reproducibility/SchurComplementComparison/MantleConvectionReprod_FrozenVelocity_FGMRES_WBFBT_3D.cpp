...
class CustomOperatorUpdater : public OperatorUpdater
{
 public:
   CustomOperatorUpdater(
       ...
       std::function< void() >                               initSolverRes )
   ...
   , maskLeft_( maskLeft )
   , initSolverRes_( initSolverRes )
   {}

   void updateBeforeSaddlePointSolve() override { initSolverRes_(); }
 ...
 private:
   ...
   std::function< void() >                        initSolverRes_;
};
...
int main( int argc, char** argv )
{
   ...
   auto ABlock = std::make_shared< stokesType::AOperatorTypeInternal >( storage_, minLevel_, maxLevel_, NewABlock, OldABlock );
   ...
   auto gradRhoRho =
       std::make_shared< stokesTypeRHS::VelocityToPressureRHSOperatorTypeInternal >( storage_, minLevel_, maxLevel_, rho_ );
   ...
   auto timeScheme = std::make_shared< BDFScheme< P2Function< real_t >, transportType::AdditiveMassType > >( 1 );
   ...
   std::shared_ptr< ABlockCGOuterLoopSolver< stokesType::AOperatorType > > ABlockOuterLoop;
   if ( blockpreconditionerType_ == 0 || blockpreconditionerType_ == 1 )
   {
      ABlockOuterLoop = std::make_shared< ABlockCGOuterLoopSolver< stokesType::AOperatorType > >(
   ...
   //    auto VectorMassInvDiagSolver = std::make_shared< ABlockInverseDiagSolver< stokesType::AOperatorType > >(
   //        parameters_, storage_, minLevel_, maxLevel_, stokesOperator_->getA(), false );
   //    VectorMassInvDiagSolver->setInvDiag( storage_, minLevel_, maxLevel_, *VectorMass );
   ...
   if ( WBFBTType_ == 1 )
   {
      SchurBFBTSolver_ = std::make_shared< SchurBFBTSolver< stokesType, VectorSqrtMassType, VectorSqrtMassType > >(
          parameters_,
          storage_,
          minLevel_,
          maxLevel_,
   ...
   {
      SchurBFBTSolver_ = std::make_shared<
          SchurBFBTSolver< stokesType, VectorSqrtMassType, VectorSqrtMassType, WrappedDivKGradType, WrappedDivKGradType > >(
          parameters_,
          storage_,
          minLevel_,
          maxLevel_,
   ...
   // Saddle Point solver hook
   auto   flag_ = hyteg::Inner | hyteg::NeumannBoundary | hyteg::FreeslipBoundary;
   auto   tmp_  = createSharedP2P1TaylorHoodFunction( "tmp", storage_, minLevel_, maxLevel_, BC_ );
   real_t initRes;
   real_t initResU;
   real_t initResP;
   ...
   std::shared_ptr< hyteg::FGMRESSolver< stokesType > > InternalFGMRES = FGMRES->getSolver();

   std::function< bool( uint_t,
                        const stokesType&,
                        const typename stokesType::srcType&,
                        const typename stokesType::srcType&,
                        uint_t,
                        real_t,
                        real_t ) >
       iterationHook = [&]( uint_t                              it,
                            const stokesType&                   A,
                            const typename stokesType::srcType& x,
                            const typename stokesType::srcType& b,
                            uint_t                              level,
                            real_t                              ApproxError,
                            real_t                              wNorm ) {
          WALBERLA_UNUSED( it );
          ...
          tmp_->assign( { 1.0, -1.0 }, { *tmp_, b }, level, flag_ );
          vertexdof::projectMean( tmp_->p(), maxLevel_ );

          real_t res  = std::sqrt( tmp_->dotGlobal( *tmp_, level, flag_ ) );
          real_t resU = std::sqrt( tmp_->uvw().dotGlobal( tmp_->uvw(), level, flag_ ) );
          real_t resP = std::sqrt( tmp_->p().dotGlobal( tmp_->p(), level, flag_ ) );
          ...
          // relative residual
          real_t res_rel  = res / initRes;
          real_t resU_rel = resU / initResU;
          real_t resP_rel = resP / initResP;

          real_t res_conv  = res / lastRes;
          real_t resU_conv = resU / lastResU;
          real_t resP_conv = resP / lastResP;
          ...
          WALBERLA_LOG_INFO_ON_ROOT( "------------- Iteration " << it << " -------------" );
          WALBERLA_LOG_INFO_ON_ROOT( "res=" << res << " ; res u=" << resU << " ; res p=" << resP << " ; res rel=" << res_rel
                                            << " ; res rel u=" << resU_rel << " ; res rel p=" << resP_rel << " ; res conv="
                                            << res_conv << " ; res conv u=" << resU_conv << " ; res conv p=" << resP_conv );

          return false;
       };

   InternalFGMRES->setGenerateSolutionForHook_( true );
   InternalFGMRES->setIterationHook( iterationHook );

   // get initial res and error
   auto& up_ = Model->getUp();
   auto& fg_ = Model->getFg();

   std::function< void() > initSolverRes = [&]() {
      iterationHook( 0, *stokesOperator_, up_, fg_, maxLevel_, real_c( 0 ), real_c( 1 ) );
   };
