...
#include "hyteg_operators/operators/mass/P2ElementwiseMassBoundary.hpp"
...
class MeshTransferUtilityWithParticles
{
 public:
   MeshTransferUtilityWithParticles()
   ...
   void transfer( const FunctionType& src, const FunctionType& dst, const uint_t levelSrc, const uint_t levelDst )
   {
      ...
      findSrcMacroWithParticle(
          *storageSrc, particleStorage_, 0.1 * MeshQuality::getMinimalEdgeLength( storageSrc, levelSrc ) );
      ...
      if ( storage.hasGlobalCells() )
      {
         for ( auto p : particleStorage )
         {
            ...
            if ( !foundByPointLocation )
            {
               ...
               WALBERLA_ABORT( "Interpolated mesh transfer failed" );
            }
         }
      }
      else
...
using MassBoundaryOperator_T = operatorgeneration::P2ElementwiseMassBoundary;
...
int main( int argc, char* argv[] )
{
   ...
   {
      cfg->readParameterFile( "./PrimitiveStorageTransfer.prm" );
   }
   else
   {
      cfg = env.config();
   ...
   MeshInfo meshInfo = MeshInfo::meshCuboid( Point3D( 0.0, 0.0, 0.0 ), Point3D( 1.0, 1.0, 1.0 ), 4u, 4u, 4u );

   SetupPrimitiveStorage setupStorage( meshInfo, walberla::MPIManager::instance()->numProcesses() );

   auto storage = std::make_shared< PrimitiveStorage >( setupStorage, 0u, true );

   const uint_t minLevel = 0u;
   const uint_t maxLevel = 4u;
   ...
   std::function< real_t( const Point3D& ) > TInterp = []( const Point3D& x ) { return std::cos( x[0] ) + std::sin( x[1] ); };

   // T.interpolate(TInterp, maxLevel, All);

   AdiosCheckpointImporter adiosImporter( checkpointFilepath, checkpointFilename, "" );
   adiosImporter.restoreFunction( T, minLevel, maxLevel );

   TP1.assign({1.0}, {T.getVertexDoFFunction()}, maxLevel, All);

   AdiosWriter adiosWriter( "./output", "TestCube", storage );

   adiosWriter.add( TP1 );
   adiosWriter.write( maxLevel );

   MeshInfo meshInfoNew = MeshInfo::meshCuboid( Point3D( 0.0, 0.0, 0.0 ), Point3D( 1.0, 1.0, 1.0 ), 8u, 8u, 8u );

   SetupPrimitiveStorage setupStorageNew( meshInfoNew, walberla::MPIManager::instance()->numProcesses() );
   loadbalancing::roundRobinVolume( setupStorageNew, walberla::MPIManager::instance()->numProcesses() );

   auto storageNew = std::make_shared< PrimitiveStorage >( setupStorageNew, 1u );

   P1Function< real_t > TNew( "TNew", storageNew, minLevel, maxLevel - 1 );

   auto meshTransferUtility = MeshTransferUtilityWithParticles< P1Function< real_t > >();
   meshTransferUtility.transfer( TP1, TNew, maxLevel, maxLevel - 1 );

   AdiosWriter adiosWriterNew( "./output", "TestCubeNew", storageNew );

   adiosWriterNew.add( TNew );
   adiosWriterNew.write( maxLevel - 1 );
