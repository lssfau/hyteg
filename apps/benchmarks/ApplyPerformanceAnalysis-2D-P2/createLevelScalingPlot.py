...
def main(datafile, perfgroup, minLevel, maxLevel):
    ...
    for messure in [perfgroup]:
        ...
        plt.plot(levelRange, outputData["Region Vertex-to-Vertex-Apply"][minLevel - min_measure_level:maxLevel + 1 - min_measure_level], '--o',
                 label="Vertex to Vertex")
        plt.plot(levelRange, outputData["Region Edge-to-Vertex-Apply"][minLevel - min_measure_level:maxLevel + 1 - min_measure_level], '--o',
                 label="Edge to Vertex")
        plt.plot(levelRange, outputData["Region Vertex-to-Edge-Apply"][minLevel - min_measure_level:maxLevel + 1 - min_measure_level], '--o',
                 label="Vertex to Edge")
        plt.plot(levelRange, outputData["Region Edge-to-Edge-Apply"][minLevel - min_measure_level:maxLevel + 1 - min_measure_level], '--o',
                 label="Edge to Edge")
