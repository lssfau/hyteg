...
#include "hyteg/p1functionspace/P2ProjectNormalOperator.hpp"
...
void applyDirichletBCRectangle( const double                channelLength,
                                ...
                                const uint_t                level,
                                P2P1TaylorHoodFunction< real_t >& u )
{
   auto inflow                = [=]( auto p ) { return p[0] <= -channelLength / 2 + 1e-14; };
...
void applyDirichletBCAnnulus( const double rmin, const double, const uint_t level, P2P1TaylorHoodFunction< real_t >& u )
{
   auto rminSq = rmin * rmin;
...
int main( int argc, char* argv[] )
{
   ...
   if ( mainConf.getParameter< bool >( "printDoFCount" ) )
   {
      ...
      {
         uint_t tmpDofStokes = numberOfGlobalDoFs< hyteg::P2P1TaylorHoodFunctionTag >( *storage, lvl );
